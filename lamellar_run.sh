...
for pe in $(seq 0 $ENDPE); do
  ...
  LAMELLAE_BACKEND="shmem" LAMELLAR_MEM_SIZE=$((1*1024*1024*1024)) LAMELLAR_THREADS=$((THREADS-1)) LAMELLAR_NUM_PES=$NUMPES LAMELLAR_PE_ID=$pe LAMELLAR_JOB_ID=$JOBID  $bin  "${@:2}"   &
  S_CORE=$(($E_CORE ))
  E_CORE=$(($S_CORE + $THREADS))
done
