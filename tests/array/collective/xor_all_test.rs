    ...
    ($array:ident, $t:ty, $len:expr, $dist:ident) =>{
       {
            ...
            for tx_size in 1..=mem_seg_len{
                ...
                for tx in (0..num_txs){
                    #[allow(unused_unsafe)]
                    reqs.push(unsafe { array.bit_xor_all(&shared_mem_region.sub_region(tx*tx_size..std::cmp::min(mem_seg_len,(tx+1)*tx_size))).spawn()});
                }
                let mut i = 0;
            ...
            for tx_size in 1..=half_len{
                ...
                for tx in (0..num_txs){
                    ...
                    #[allow(unused_unsafe)]
                    reqs.push(unsafe { array.bit_xor_all(&shared_mem_region.sub_region(tx*tx_size..std::cmp::min(half_len,(tx+1)*tx_size))).spawn()});
                }
            ...
            for pe in 0..num_pes{
                ...
                for tx_size in 1..len{
                    ...
                    for tx in (0..num_txs){
                        ...
                        #[allow(unused_unsafe)]
                        reqs.push(unsafe { sub_array.bit_xor_all(&shared_mem_region.sub_region(tx*tx_size..std::cmp::min(len,(tx+1)*tx_size))).spawn()});
                    }
                    // array.wait_all();
                    // sub_array.barrier();
                    let mut i = 0;
...
fn main() {
    ...
    match array.as_str() {
        "UnsafeArray" => match elem.as_str() {
            ...
            "f32" => bit_xor_all_test!(UnsafeArray, f32, len, dist_type),
            "f64" => bit_xor_all_test!(UnsafeArray, f64, len, dist_type),
        ...
        "AtomicArray" => match elem.as_str() {
            ...
            "f32" => bit_xor_all_test!(AtomicArray, f32, len, dist_type),
            "f64" => bit_xor_all_test!(AtomicArray, f64, len, dist_type),
            _ => eprintln!("unsupported element type"),
        },
        // "LocalLockArray" => match elem.as_str() {
        //     "u8" => bit_xor_all_test!(LocalLockArray, u8, len, dist_type),
        //     "u16" => bit_xor_all_test!(LocalLockArray, u16, len, dist_type),
        //     "u32" => bit_xor_all_test!(LocalLockArray, u32, len, dist_type),
        //     "u64" => bit_xor_all_test!(LocalLockArray, u64, len, dist_type),
        //     "u128" => bit_xor_all_test!(LocalLockArray, u128, len, dist_type),
        //     "usize" => bit_xor_all_test!(LocalLockArray, usize, len, dist_type),
        //     "i8" => bit_xor_all_test!(LocalLockArray, i8, len, dist_type),
        //     "i16" => bit_xor_all_test!(LocalLockArray, i16, len, dist_type),
        //     "i32" => bit_xor_all_test!(LocalLockArray, i32, len, dist_type),
        //     "i64" => bit_xor_all_test!(LocalLockArray, i64, len, dist_type),
        //     "i128" => bit_xor_all_test!(LocalLockArray, i128, len, dist_type),
        //     "isize" => bit_xor_all_test!(LocalLockArray, isize, len, dist_type),
        //     "f32" => bit_xor_all_test!(LocalLockArray, f32, len, dist_type),
        //     "f64" => bit_xor_all_test!(LocalLockArray, f64, len, dist_type),
        //     _ => eprintln!("unsupported element type"),
        // },
        ...
        //     "f32" => bit_xor_all_test!(GlobalLockArray, f32, len, dist_type),
        //     "f64" => bit_xor_all_test!(GlobalLockArray, f64, len, dist_type),
        //     _ => {} //eprintln!("unsupported element type"),
        // },
