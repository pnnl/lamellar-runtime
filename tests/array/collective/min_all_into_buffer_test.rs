...
macro_rules! min_all_into_buffer_test{
    ($array:ident, $t:ty, $len:expr, $dist:ident) =>{
       {
            ...
            for tx_size in 1..=mem_seg_len{
                ...
                for tx in 0..num_txs{
                    ...
                    #[allow(unused_unsafe)]
                    unsafe { array_or_lock!($array, array, _lock).min_all_into_buffer(tx*tx_size, std::cmp::min(mem_seg_len,(tx+1)*tx_size) - tx*tx_size, buffer).block(); }
                    buffer = buf;
                }
                array.barrier();
...
fn main() {
    ...
    match array.as_str() {
        ...
        "GlobalLockArray" => match elem.as_str() {
            "u8" => min_all_into_buffer_test!(GlobalLockArray, u8, len, dist_type),
            "u16" => min_all_into_buffer_test!(GlobalLockArray, u16, len, dist_type),
            "u32" => min_all_into_buffer_test!(GlobalLockArray, u32, len, dist_type),
            "u64" => min_all_into_buffer_test!(GlobalLockArray, u64, len, dist_type),
            "u128" => min_all_into_buffer_test!(GlobalLockArray, u128, len, dist_type),
            "usize" => min_all_into_buffer_test!(GlobalLockArray, usize, len, dist_type),
            "i8" => min_all_into_buffer_test!(GlobalLockArray, i8, len, dist_type),
            "i16" => min_all_into_buffer_test!(GlobalLockArray, i16, len, dist_type),
            "i32" => min_all_into_buffer_test!(GlobalLockArray, i32, len, dist_type),
            "i64" => min_all_into_buffer_test!(GlobalLockArray, i64, len, dist_type),
            "i128" => min_all_into_buffer_test!(GlobalLockArray, i128, len, dist_type),
            "isize" => min_all_into_buffer_test!(GlobalLockArray, isize, len, dist_type),
            "f32" => min_all_into_buffer_test!(GlobalLockArray, f32, len, dist_type),
            "f64" => min_all_into_buffer_test!(GlobalLockArray, f64, len, dist_type),
            _ => {} //eprintln!("unsupported element type"),
        },
