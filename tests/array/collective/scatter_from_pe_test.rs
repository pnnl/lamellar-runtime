    ...
    ($array:ident, $t:ty, $len:expr, $dist:ident) =>{
       {
            ...
            for tx_size in (num_pes..=mem_seg_len).step_by(num_pes){
                ...
                for req in reqs.drain(..){
                    ...
                    for elem in buf.as_slice().iter(){
                        if ( (( (i/ req.1 * num_pes + my_pe) *  req.1 + i % req.1) as $t  - elem) as f32).abs() > 0.0001 {
                            eprintln!("[{:?}] {:?} {:?} {:?} {:?} {:?}",my_pe, i as $t, ( (i/ req.1 * num_pes + my_pe) *  req.1 + i % req.1) as $t, elem,( (( (i/ req.1 * num_pes + my_pe) *  req.1 + i % req.1) as $t  - elem) as f32).abs(), req.1);
                            success = false;
                        }
                        i+=1;
                    }
            ...
            world.barrier();
            // sub_array.print();
            for tx_size in (num_pes..=half_len).step_by(num_pes){
                ...
                for req in reqs.drain(..){
                    ...
                    for elem in buf.as_slice().iter(){
                        if ( (( (i/ req.1 * num_pes + my_pe) *  req.1 + i % req.1) as $t  - elem) as f32).abs() > 0.0001 {
                            eprintln!("{:?} {:?} {:?}",i as $t,elem,( (( (i/ req.1 * num_pes + my_pe) *  req.1 + i % req.1) as $t  - elem) as f32).abs());
                            success = false;
                        }
                        i+=1;
                    }
            ...
            for pe in 0..num_pes{
                ...
                for tx_size in (num_pes..len).step_by(num_pes){
...
fn main() {
    ...
    match array.as_str() {
        "UnsafeArray" => match elem.as_str() {
            "u8" => scatter_from_pe_test!(UnsafeArray, u8, len, dist_type),
            // "u16" => scatter_from_pe_test!(UnsafeArray, u16, len, dist_type),
            // "u32" => scatter_from_pe_test!(UnsafeArray, u32, len, dist_type),
            // "u64" => scatter_from_pe_test!(UnsafeArray, u64, len, dist_type),
            // "u128" => scatter_from_pe_test!(UnsafeArray, u128, len, dist_type),
            // "usize" => scatter_from_pe_test!(UnsafeArray, usize, len, dist_type),
            // "i8" => scatter_from_pe_test!(UnsafeArray, i8, len, dist_type),
            // "i16" => scatter_from_pe_test!(UnsafeArray, i16, len, dist_type),
            // "i32" => scatter_from_pe_test!(UnsafeArray, i32, len, dist_type),
            // "i64" => scatter_from_pe_test!(UnsafeArray, i64, len, dist_type),
            // "i128" => scatter_from_pe_test!(UnsafeArray, i128, len, dist_type),
            // "isize" => scatter_from_pe_test!(UnsafeArray, isize, len, dist_type),
            // "f32" => scatter_from_pe_test!(UnsafeArray, f32, len, dist_type),
            // "f64" => scatter_from_pe_test!(UnsafeArray, f64, len, dist_type),
            _ => eprintln!("unsupported element type"),
        },
        "AtomicArray" => match elem.as_str() {
            "u8" => scatter_from_pe_test!(AtomicArray, u8, len, dist_type),
            // "u16" => scatter_from_pe_test!(AtomicArray, u16, len, dist_type),
            // "u32" => scatter_from_pe_test!(AtomicArray, u32, len, dist_type),
            // "u64" => scatter_from_pe_test!(AtomicArray, u64, len, dist_type),
            // "u128" => scatter_from_pe_test!(AtomicArray, u128, len, dist_type),
            // "usize" => scatter_from_pe_test!(AtomicArray, usize, len, dist_type),
            // "i8" => scatter_from_pe_test!(AtomicArray, i8, len, dist_type),
            // "i16" => scatter_from_pe_test!(AtomicArray, i16, len, dist_type),
            // "i32" => scatter_from_pe_test!(AtomicArray, i32, len, dist_type),
            // "i64" => scatter_from_pe_test!(AtomicArray, i64, len, dist_type),
            // "i128" => scatter_from_pe_test!(AtomicArray, i128, len, dist_type),
            // "isize" => scatter_from_pe_test!(AtomicArray, isize, len, dist_type),
            // "f32" => scatter_from_pe_test!(AtomicArray, f32, len, dist_type),
            // "f64" => scatter_from_pe_test!(AtomicArray, f64, len, dist_type),
            _ => eprintln!("unsupported element type"),
        },
        // "LocalLockArray" => match elem.as_str() {
        //     "u8" => scatter_from_pe_test!(LocalLockArray, u8, len, dist_type),
        //     "u16" => scatter_from_pe_test!(LocalLockArray, u16, len, dist_type),
        //     "u32" => scatter_from_pe_test!(LocalLockArray, u32, len, dist_type),
        //     "u64" => scatter_from_pe_test!(LocalLockArray, u64, len, dist_type),
        //     "u128" => scatter_from_pe_test!(LocalLockArray, u128, len, dist_type),
        //     "usize" => scatter_from_pe_test!(LocalLockArray, usize, len, dist_type),
        //     "i8" => scatter_from_pe_test!(LocalLockArray, i8, len, dist_type),
        //     "i16" => scatter_from_pe_test!(LocalLockArray, i16, len, dist_type),
        //     "i32" => scatter_from_pe_test!(LocalLockArray, i32, len, dist_type),
        //     "i64" => scatter_from_pe_test!(LocalLockArray, i64, len, dist_type),
        //     "i128" => scatter_from_pe_test!(LocalLockArray, i128, len, dist_type),
        //     "isize" => scatter_from_pe_test!(LocalLockArray, isize, len, dist_type),
        //     "f32" => scatter_from_pe_test!(LocalLockArray, f32, len, dist_type),
