            ...
            for tx_size in (1..=mem_seg_len).step_by(num_pes){
                ...
                for req in reqs.drain(..){
                    ...
                    for elem in buf.as_slice().iter(){
                        if ( ((( (i/ req.1 * num_pes + my_pe) *  req.1 + i % req.1) * num_pes) as $t  - elem) as f32).abs() > 0.0001 {
                            eprintln!("{:?} {:?} {:?}",i as $t,elem,( ((( (i/ req.1 * num_pes + my_pe) *  req.1 + i % req.1) * num_pes) as $t  - elem) as f32).abs());
                            success = false;
                        }
                        i+=1;
                    }
