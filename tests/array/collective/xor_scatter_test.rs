    ...
    ($array:ident, $t:ty, $len:expr, $dist:ident) =>{
       {
            ...
            for tx_size in (1..=mem_seg_len).step_by(num_pes){
                ...
                for tx in (0..num_txs){
                    ...
                    #[allow(unused_unsafe)]
                    reqs.push((unsafe { array.bit_xor_scatter(&shared_mem_region.sub_region(tx*tx_size..std::cmp::min(mem_seg_len,(tx+1)*tx_size)), chunk_size).spawn()}, chunk_size));
                }
                let mut i = 0;
            ...
            for tx_size in 1..=half_len{
                ...
                for tx in (0..num_txs){
                    ...
                    #[allow(unused_unsafe)]
                    reqs.push((unsafe { array.bit_xor_scatter(&shared_mem_region.sub_region(tx*tx_size..std::cmp::min(half_len,(tx+1)*tx_size)), chunk_size).spawn()}, chunk_size));
                }
            ...
            for pe in 0..num_pes{
                ...
                for tx_size in 1..len{
                    ...
                    for tx in (0..num_txs){
                        ...
                        #[allow(unused_unsafe)]
                        reqs.push((unsafe { sub_array.bit_xor_scatter(&shared_mem_region.sub_region(tx*tx_size..std::cmp::min(len,(tx+1)*tx_size)), chunk_size).spawn()}, chunk_size));
                    }
                    // array.wait_all();
                    // sub_array.barrier();
                    let mut i = 0;
...
fn main() {
    ...
    match array.as_str() {
        "UnsafeArray" => match elem.as_str() {
            "u8" => bit_xor_scatter_test!(UnsafeArray, u8, len, dist_type),
            "u16" => bit_xor_scatter_test!(UnsafeArray, u16, len, dist_type),
            "u32" => bit_xor_scatter_test!(UnsafeArray, u32, len, dist_type),
            "u64" => bit_xor_scatter_test!(UnsafeArray, u64, len, dist_type),
            "u128" => bit_xor_scatter_test!(UnsafeArray, u128, len, dist_type),
            "usize" => bit_xor_scatter_test!(UnsafeArray, usize, len, dist_type),
            "i8" => bit_xor_scatter_test!(UnsafeArray, i8, len, dist_type),
            "i16" => bit_xor_scatter_test!(UnsafeArray, i16, len, dist_type),
            "i32" => bit_xor_scatter_test!(UnsafeArray, i32, len, dist_type),
            "i64" => bit_xor_scatter_test!(UnsafeArray, i64, len, dist_type),
            "i128" => bit_xor_scatter_test!(UnsafeArray, i128, len, dist_type),
            "isize" => bit_xor_scatter_test!(UnsafeArray, isize, len, dist_type),
            "f32" => bit_xor_scatter_test!(UnsafeArray, f32, len, dist_type),
            "f64" => bit_xor_scatter_test!(UnsafeArray, f64, len, dist_type),
            _ => eprintln!("unsupported element type"),
        },
        "AtomicArray" => match elem.as_str() {
            "u8" => bit_xor_scatter_test!(AtomicArray, u8, len, dist_type),
            "u16" => bit_xor_scatter_test!(AtomicArray, u16, len, dist_type),
            "u32" => bit_xor_scatter_test!(AtomicArray, u32, len, dist_type),
            "u64" => bit_xor_scatter_test!(AtomicArray, u64, len, dist_type),
            "u128" => bit_xor_scatter_test!(AtomicArray, u128, len, dist_type),
            "usize" => bit_xor_scatter_test!(AtomicArray, usize, len, dist_type),
            "i8" => bit_xor_scatter_test!(AtomicArray, i8, len, dist_type),
            "i16" => bit_xor_scatter_test!(AtomicArray, i16, len, dist_type),
            "i32" => bit_xor_scatter_test!(AtomicArray, i32, len, dist_type),
            ...
            "f32" => bit_xor_scatter_test!(AtomicArray, f32, len, dist_type),
            "f64" => bit_xor_scatter_test!(AtomicArray, f64, len, dist_type),
            _ => eprintln!("unsupported element type"),
        },
