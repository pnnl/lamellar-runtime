    ...
    ($array:ident, $t:ty, $len:expr, $dist:ident) =>{
       {
            ...
            for tx_size in (num_pes..=mem_seg_len).step_by(num_pes){
                ...
                for tx in (0..num_txs){
                    ...
                    #[allow(unused_unsafe)]
                    reqs.push((unsafe { array_or_lock!($array, array, _lock).prod_scatter(tx * tx_size, interval_len).spawn()}, chunk_size));
                }
...
fn main() {
    ...
    match array.as_str() {
        ...
        "GlobalLockArray" => match elem.as_str() {
            "u8" => prod_scatter_test!(GlobalLockArray, u8, len, dist_type),
            "u16" => prod_scatter_test!(GlobalLockArray, u16, len, dist_type),
            "u32" => prod_scatter_test!(GlobalLockArray, u32, len, dist_type),
            "u64" => prod_scatter_test!(GlobalLockArray, u64, len, dist_type),
            "u128" => prod_scatter_test!(GlobalLockArray, u128, len, dist_type),
            "usize" => prod_scatter_test!(GlobalLockArray, usize, len, dist_type),
            "i8" => prod_scatter_test!(GlobalLockArray, i8, len, dist_type),
            "i16" => prod_scatter_test!(GlobalLockArray, i16, len, dist_type),
            "i32" => prod_scatter_test!(GlobalLockArray, i32, len, dist_type),
            "i64" => prod_scatter_test!(GlobalLockArray, i64, len, dist_type),
            "i128" => prod_scatter_test!(GlobalLockArray, i128, len, dist_type),
            "isize" => prod_scatter_test!(GlobalLockArray, isize, len, dist_type),
            "f32" => prod_scatter_test!(GlobalLockArray, f32, len, dist_type),
            "f64" => prod_scatter_test!(GlobalLockArray, f64, len, dist_type),
            _ => eprintln!("unsupported element type"),
        },
