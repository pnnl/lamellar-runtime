    ...
    ($array:ident, $t:ty, $len:expr, $dist:ident) =>{
       {
            ...
            for tx_size in (num_pes..=mem_seg_len).step_by(num_pes){
                ...
                for tx in (0..num_txs){
                    ...
                    #[allow(unused_unsafe)]
                    reqs.push((unsafe { array_or_lock!($array, array, _lock).min_scatter(tx * tx_size, interval_len).spawn()}, chunk_size));
                }
                for req in reqs.drain(..){
                    let buf =req.0.block();
                    for elem in buf.as_slice().iter(){
                        if *elem != 0 as $t {
                            eprintln!("expected {:?} got {:?}", 0 as $t, elem);
                            success = false;
                        }
                    }
                }
            ...
            //             #[allow(unused_unsafe)]
            //             reqs.push((unsafe { sub_array.min_scatter(tx * tx_size, chunk_size).spawn()}, chunk_size));
            //         }
            //         // array.wait_all();
            //         // sub_array.barrier();
            //         for req in reqs.drain(..){
