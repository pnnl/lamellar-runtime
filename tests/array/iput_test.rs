...
//             let block_array: $array::<$t> = $array::<$t>::new(world.team(), array_total_len, $dist).into(); //convert into abstract LamellarArray, distributed len is total_len
    ...
    ($array:ident, $t:ty, $len:expr, $dist:ident) =>{
       {
            ...
            for tx_size in 1..=mem_seg_len{
                ...
                for tx in (my_pe..num_txs).step_by(num_pes){
                    ...
                    array.iput(tx*tx_size,&shared_mem_region.sub_region(tx*tx_size..std::cmp::min(mem_seg_len,(tx+1)*tx_size)));
                }
                array.barrier();
            ...
            for tx_size in 1..=half_len{
                ...
                for tx in (my_pe..num_txs).step_by(num_pes){
                    ...
                    sub_array.iput(tx*tx_size,&shared_mem_region.sub_region(tx*tx_size..std::cmp::min(half_len,(tx+1)*tx_size)));
                }
                sub_array.barrier();
            ...
            for pe in 0..num_pes{
                ...
                for tx_size in 1..len{
                    ...
                    for tx in (my_pe..num_txs).step_by(num_pes){
                        ...
                        sub_array.iput(tx*tx_size,&shared_mem_region.sub_region(tx*tx_size..std::cmp::min(len,(tx+1)*tx_size)));
                    }
                    sub_array.barrier();
                    for (i,elem) in sub_array.ser_iter().into_iter().enumerate().take( num_txs * tx_size){
                        if ((i as $t - *elem) as f32).abs() > 0.0001 {
...
fn main() {
    ...
    match array.as_str(){
        "UnsafeArray" => {
            match elem.as_str() {
                "u8" => iput_test!(UnsafeArray,u8,len,dist_type),
                "u16" => iput_test!(UnsafeArray,u16,len,dist_type),
                "u32" => iput_test!(UnsafeArray,u32,len,dist_type),
                "u64" => iput_test!(UnsafeArray,u64,len,dist_type),
                "u128" => iput_test!(UnsafeArray,u128,len,dist_type),
                "usize" => iput_test!(UnsafeArray,usize,len,dist_type),
                "i8" => iput_test!(UnsafeArray,u8,len,dist_type),
                "i16" => iput_test!(UnsafeArray,u16,len,dist_type),
                "i32" => iput_test!(UnsafeArray,u32,len,dist_type),
                "i64" => iput_test!(UnsafeArray,u64,len,dist_type),
                "i128" => iput_test!(UnsafeArray,u128,len,dist_type),
                "isize" => iput_test!(UnsafeArray,usize,len,dist_type),
                "f32" => iput_test!(UnsafeArray,f32,len,dist_type),
                "f64" => iput_test!(UnsafeArray,f64,len,dist_type),
                _ =>  eprintln!("unsupported element type"),
            }
        }
        "AtomicArray" => {
            match elem.as_str() {
                "u8" => iput_test!(AtomicArray,u8,len,dist_type),
                "u16" => iput_test!(AtomicArray,u16,len,dist_type),
                "u32" => iput_test!(AtomicArray,u32,len,dist_type),
                "u64" => iput_test!(AtomicArray,u64,len,dist_type),
                "u128" => iput_test!(AtomicArray,u128,len,dist_type),
                "usize" => iput_test!(AtomicArray,usize,len,dist_type),
                "i8" => iput_test!(AtomicArray,u8,len,dist_type),
                "i16" => iput_test!(AtomicArray,u16,len,dist_type),
                "i32" => iput_test!(AtomicArray,u32,len,dist_type),
                "i64" => iput_test!(AtomicArray,u64,len,dist_type),
                "i128" => iput_test!(AtomicArray,u128,len,dist_type),
                "isize" => iput_test!(AtomicArray,usize,len,dist_type),
                "f32" => iput_test!(AtomicArray,f32,len,dist_type),
                "f64" => iput_test!(AtomicArray,f64,len,dist_type),
                _ =>  eprintln!("unsupported element type"),
            }
        }
        "CollectiveAtomicArray" => {
            match elem.as_str() {
                "u8" => iput_test!(CollectiveAtomicArray,u8,len,dist_type),
                "u16" => iput_test!(CollectiveAtomicArray,u16,len,dist_type),
                "u32" => iput_test!(CollectiveAtomicArray,u32,len,dist_type),
                "u64" => iput_test!(CollectiveAtomicArray,u64,len,dist_type),
                "u128" => iput_test!(CollectiveAtomicArray,u128,len,dist_type),
                "usize" => iput_test!(CollectiveAtomicArray,usize,len,dist_type),
                "i8" => iput_test!(CollectiveAtomicArray,u8,len,dist_type),
                "i16" => iput_test!(CollectiveAtomicArray,u16,len,dist_type),
                "i32" => iput_test!(CollectiveAtomicArray,u32,len,dist_type),
                "i64" => iput_test!(CollectiveAtomicArray,u64,len,dist_type),
                "i128" => iput_test!(CollectiveAtomicArray,u128,len,dist_type),
                "isize" => iput_test!(CollectiveAtomicArray,usize,len,dist_type),
                "f32" => iput_test!(CollectiveAtomicArray,f32,len,dist_type),
                "f64" => iput_test!(CollectiveAtomicArray,f64,len,dist_type),
                _ =>  eprintln!("unsupported element type"),
            }
        }
