    ...
    ($array:ident, $t:ty, $len:expr, $dist:ident) =>{
       {
            ...
            for tx_size in 1..=mem_seg_len{
                ...
                for tx in (0..num_txs){
                    ...
                    unsafe {array.get(tx*tx_size,&shared_mem_region.sub_region(tx*tx_size..std::cmp::min(mem_seg_len,(tx+1)*tx_size))).block();}
                }
                array.barrier();
            ...
            for tx_size in 1..=half_len{
                ...
                for tx in (0..num_txs){
                    ...
                    unsafe {sub_array.get(tx*tx_size,&shared_mem_region.sub_region(tx*tx_size..std::cmp::min(half_len,(tx+1)*tx_size))).block();}
                }
                sub_array.barrier();
                // unsafe{println!("{:?}",shared_mem_region.as_slice());}
                unsafe{
            ...
            for pe in 0..num_pes{
                ...
                for tx_size in 1..len{
                    ...
                    for tx in (0..num_txs){
                        ...
                        unsafe {sub_array.get(tx*tx_size,&shared_mem_region.sub_region(tx*tx_size..std::cmp::min(len,(tx+1)*tx_size))).block();}
                    }
                    sub_array.barrier();
                    unsafe{
                        for (i,elem) in shared_mem_region.as_slice().iter().enumerate().take( num_txs * tx_size){
