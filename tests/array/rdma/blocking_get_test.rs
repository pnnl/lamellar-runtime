...
macro_rules! blocking_get_test{
    ($array:ident, $t:ty, $len:expr, $dist:ident) =>{{
        ...
        for tx in (0..mem_seg_len){
            #[allow(unused_unsafe)]
            let elem = unsafe { array.blocking_get(tx) };
            if ((tx as $t - elem) as f32).abs() > 0.0001 {
                eprintln!("{:?} {:?} {:?}",tx as $t,elem,((tx as $t - elem) as f32).abs());
                success = false;
            }
        }
        ...
        for tx in (0..half_len){
            #[allow(unused_unsafe)]
            let elem = unsafe { sub_array.blocking_get(tx) };
            if ((tx as $t - elem) as f32).abs() > 0.0001 {
                eprintln!("{:?} {:?} {:?}",tx as $t,elem,((tx as $t - elem) as f32).abs());
                success = false;
            }
...
fn main() {
    ...
    match array.as_str() {
        ...
        "GlobalLockArray" => match elem.as_str() {
            "u8" => blocking_get_test!(GlobalLockArray, u8, len, dist_type),
            "u16" => blocking_get_test!(GlobalLockArray, u16, len, dist_type),
            "u32" => blocking_get_test!(GlobalLockArray, u32, len, dist_type),
            "u64" => blocking_get_test!(GlobalLockArray, u64, len, dist_type),
            "u128" => blocking_get_test!(GlobalLockArray, u128, len, dist_type),
            "usize" => blocking_get_test!(GlobalLockArray, usize, len, dist_type),
            "i8" => blocking_get_test!(GlobalLockArray, i8, len, dist_type),
            "i16" => blocking_get_test!(GlobalLockArray, i16, len, dist_type),
            "i32" => blocking_get_test!(GlobalLockArray, i32, len, dist_type),
            "i64" => blocking_get_test!(GlobalLockArray, i64, len, dist_type),
            "i128" => blocking_get_test!(GlobalLockArray, i128, len, dist_type),
            "isize" => blocking_get_test!(GlobalLockArray, isize, len, dist_type),
            "f32" => blocking_get_test!(GlobalLockArray, f32, len, dist_type),
            "f64" => blocking_get_test!(GlobalLockArray, f64, len, dist_type),
            _ => {} //eprintln!("unsupported element type"),
        },
