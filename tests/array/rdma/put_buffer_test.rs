    ...
    ($array:ident, $t:ty, $len:expr, $dist:ident) =>{
       {
            ...
            for tx_size in 1..=half_len{
                ...
                for tx in (my_pe..num_txs).step_by(num_pes){
                    ...
                    #[allow(unused_unsafe)]
                    unsafe {let _ = sub_array.put_buffer(tx*tx_size,&shared_mem_region.sub_region(tx*tx_size..std::cmp::min(half_len,(tx+1)*tx_size))).spawn();}
                }
                array.wait_all();
                sub_array.barrier();
                #[allow(unused_unsafe)]
            ...
            for pe in 0..num_pes{
                ...
                for tx_size in 1..len{
                    ...
                    for tx in (my_pe..num_txs).step_by(num_pes){
                        ...
                        #[allow(unused_unsafe)]
                        unsafe {let _ = sub_array.put_buffer(tx*tx_size,&shared_mem_region.sub_region(tx*tx_size..std::cmp::min(len,(tx+1)*tx_size))).spawn();}
                    }
                    array.wait_all();
                    sub_array.barrier();
                    #[allow(unused_unsafe)]
