use lamellar::{
    ActiveMessaging, LamellarAM, LamellarArray,RegisteredMemoryRegion
};
...
fn main(){
    ...
    unsafe { 
        ...
        for i in len_per_pe..total_len{ //finish filling out the local buf
            local[i]=i+1;
        }
    ...
    if my_pe == 0{
        ...
        println!("sum: {:?} ({:?}) {:?} ({:?})",sum, dist_time, local_sum,local_time);
