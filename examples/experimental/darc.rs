...
fn main() {
    ...
    let global_lock = GlobalRwDarc::new(world.team(),3242).unwrap();
    let read_lock = global_lock.read();
    println!("i have the read lock!!!! {:?}",my_pe);
    drop(read_lock);
    let write_lock = global_lock.write();
    println!("i have the write lock!!!! {:?}",my_pe);
    std::thread::sleep(std::time::Duration::from_secs(2));
    drop(write_lock);

    // let local_test = LocalRwDarc::new(world.team(),10).unwrap();
