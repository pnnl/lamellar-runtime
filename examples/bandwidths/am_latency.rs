...
fn main() {
    ...
    // ---- all-to-neighbor: exec_am_pe().block() ----
    world.barrier();
    if my_pe == 0 {
        println!();
        println!("# All-to-neighbor exec_am_pe().block()  (PE i -> PE (i+1)%N, concurrent)");
        println!(
            "# {:>12}  {:>9}  {:>9}  {:>9}  {:>9}  {:>9}",
            "size_B", "min_us", "p50_us", "p95_us", "p99_us", "max_us"
        );
    ...
    let neighbor = (my_pe + 1) % num_pes;

    for &size in &sizes[..8] {
        let data = vec![42u8; size];

        for _ in 0..WARMUP {
            world.exec_am_pe(neighbor, LatencyAM { data: data.clone() }).block();
        }
        world.barrier();
    ...
    // ---- all-to-neighbor: spawn_am_pe().block() ----
    world.barrier();
    if my_pe == 0 {
        println!();
        println!("# All-to-neighbor spawn_am_pe().block()  (PE i -> PE (i+1)%N, concurrent)");
        println!(
            "# {:>12}  {:>9}  {:>9}  {:>9}  {:>9}  {:>9}",
            "size_B", "min_us", "p50_us", "p95_us", "p99_us", "max_us"
        );
    ...
    for &size in &sizes[..8] {
        ...
        for _ in 0..WARMUP {
            world.spawn_am_pe(neighbor, LatencyAM { data: data.clone() }).block();
        }
        world.barrier();
