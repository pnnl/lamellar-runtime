use lamellar::{ActiveMessaging,RemoteMemoryRegion, LocalMemoryRegion, Darc, BlockedArch, StridedArch};
    ...
    fn exec(){
        ...
        if self.depth > 0 {
            for _i in 0..self.width {
                ...
                lamellar::team.exec_am_pe(pe, DataAM{darc: self.darc.clone(), depth: self.depth - 1, width: self.width, path: path.clone()});
            }
        }
    }
}
...
fn main(){
    ...
    for _i in 0..width {
        let pe = pes.sample(&mut rng);
        world.exec_am_pe(pe, DataAM{darc: darc.clone(), depth: 5, width: width, path: vec![my_pe]});
    }
    world.wait_all();
    world.barrier(); 
    ...
    for _i in 0..width {
        ...
        first_half_team.exec_am_pe(pe, DataAM{darc: darc.clone(), depth: 5, width: width, path: vec![my_pe]});
        odd_team.exec_am_pe(pe, DataAM{darc: darc.clone(), depth: 5, width: width, path: vec![my_pe]});
    }
    world.wait_all();
    world.barrier();
    println!("time: {:?}",s.elapsed().as_secs_f64()); 
