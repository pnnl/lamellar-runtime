...
fn for_each_with_schedule(schedule: Schedule, array: &AtomicArray<usize>, thread_cnts: Arc<Mutex<HashMap<ThreadId, usize>>>){
    let timer = Instant::now();
    let tc = thread_cnts.clone();
    array.local_iter().filter(|e| e.load()%2 == 0).for_each_with_schedule(schedule, move |e| {
...
fn reduce_with_schedule(schedule: Schedule, array: &AtomicArray<usize>, thread_cnts: Arc<Mutex<HashMap<ThreadId, usize>>>){
    let timer = Instant::now();
    let tc = thread_cnts.clone();
    let result = array.block_on(array.local_iter().filter(|e| e.load()%2 == 0).map(|e| e.load()).reduce_with_schedule(schedule, |e1, e2| e1 + e2));
    array.barrier();
    println!("elapsed time {:?}", timer.elapsed().as_secs_f64());
    println!("reduced {:?}", result);
}

fn collect_with_schedule(schedule: Schedule, array: &AtomicArray<usize>, thread_cnts: Arc<Mutex<HashMap<ThreadId, usize>>>){
    let timer = Instant::now();
    let tc = thread_cnts.clone();
...
fn count_with_schedule(schedule: Schedule, array: &AtomicArray<usize>, thread_cnts: Arc<Mutex<HashMap<ThreadId, usize>>>){
    let timer = Instant::now();
    let tc = thread_cnts.clone();
    let result = array.block_on(array.local_iter().filter(|e| e.load()%2 == 0).count_with_schedule::<>(schedule));
    array.barrier();
    println!("elapsed time {:?}", timer.elapsed().as_secs_f64());
