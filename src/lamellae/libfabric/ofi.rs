...
impl Ofi {
    pub(crate) fn new(provider: Option<&str>, domain: Option<&str>) -> Result<Self, libfabric::error::Error> {
            ...
            .find(|e| 
                if let Some(prov) = provider {
                    if let Some(dom) = domain {
                        e.get_fabric_attr().get_prov_name().split(';').any(|s| s == prov) && e.get_domain_attr().name.split(';').any(|s| s == dom)
                    }
                    else {
                        e.get_fabric_attr().get_prov_name().split(';').any(|s| s == prov)
                    }
                }
                else {
                    if let Some(dom) = domain {
                        e.get_domain_attr().name.split(';').any(|s| s == dom)
                    }
                    else {
        ...
        let cq = libfabric::cq::CompletionQueueBuilder::new()
            ...
            .size(info_entry.get_rx_attr().get_size())
            .build(&domain)?;
        ...
        let mapped_addresses = av.insert(&unmapped_addresses, libfabric::enums::AVOptions::new())?;
    ...
    fn create_mc_group(&self, pes: &[usize]) -> Result<libfabric::comm::collective::MulticastGroupCollective, libfabric::error::Error> {
        ...
        let mut av_set = libfabric::av::AddressVectorSetBuilder::new(&self.av)
            .count(pes.len())
        ...
        let mut ctx = libfabric::Context::new();
    ...
    fn wait_for_join_event<T>(&self, ctx: &T) -> Result<(), libfabric::error::Error> {
        loop {
    ...
    fn wait_for_completion<T>(&self, ctx: &T) -> Result<(), libfabric::error::Error> {
    ...
    fn exchange_mr_info(&self, addr: usize, len: usize, key: &libfabric::mr::MemoryRegionKey, pes: &[usize]) -> Result<Vec<libfabric::iovec::RmaIoVec>, libfabric::error::Error> {
        ...
        self.ep.allgather_with_context(std::slice::from_mut(&mut my_rma_iov), &mut libfabric::mr::default_desc(), &mut all_rma_iovs, &mut libfabric::mr::default_desc(), &mc, libfabric::enums::TferOptions::new(), &mut ctx)?;
    ...
    pub(crate) fn sub_alloc(&self, pes: &[usize], size: usize) -> Result<usize, libfabric::error::Error> {
        ...
        let mr = libfabric::mr::MemoryRegionBuilder::new(&mem)
    ...
    pub(crate) unsafe fn put<T: Remote>(&self, pe:usize, src_addr: &[T], dst_addr:usize, sync: bool) -> Result<(), libfabric::error::Error>{
        ...
        let cntr_order = if std::mem::size_of_val(src_addr) < self.info_entry.get_tx_attr().get_inject_size() {
            ...
            while curr_idx < src_addr.len() {
                let msg_len = std::cmp::min(src_addr.len() - curr_idx, self.info_entry.get_ep_attr().get_max_msg_size()); 
    ...
    pub(crate) unsafe fn get<T: Remote>(&self, pe:usize, src_addr:usize, dst_addr: &mut [T], sync: bool) -> Result<(), libfabric::error::Error> {
        let (offset, mut desc,  remote_alloc_info) = {
            let table = self.alloc_manager.mr_info_table.read();
            let alloc_info = table
                .iter()
        ...
        while curr_idx < dst_addr.len() {
            let msg_len = std::cmp::min(dst_addr.len() - curr_idx,  self.info_entry.get_ep_attr().get_max_msg_size());
