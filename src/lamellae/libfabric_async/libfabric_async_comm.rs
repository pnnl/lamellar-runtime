...
static LIBFABASYNC_MAGIC_8: u64 = 0b1101001110111100011111001001100100111110011001100011110111001011;
static LIBFABASYNC_MAGIC_4: u32 = 0b10001111100100110010011111001100;
static LIBFABASYNC_MAGIC_2: u16 = 0b1100100110010011;
static LIBFABASYNC_MAGIC_1: u8 = 0b10011001;
...
impl LibFabAsyncComm {
    pub(crate) fn new(provider: Option<&str>, domain: Option<&str>) -> Result<Self, libfabric::error::Error> {
        ...
        let ofi = OfiAsync::new(provider, domain)?;
    ...
    fn init_buffer<T>(&self, dst_addr: &mut [T]) {
        ...
        unsafe {
            if bytes_len % std::mem::size_of::<u64>() == 0 {
                self.fill_buffer(dst_addr, LIBFABASYNC_MAGIC_8);
            } else if bytes_len % std::mem::size_of::<u32>() == 0 {
                self.fill_buffer(dst_addr, LIBFABASYNC_MAGIC_4);
            } else if bytes_len % std::mem::size_of::<u16>() == 0 {
                self.fill_buffer(dst_addr, LIBFABASYNC_MAGIC_2);
            } else {
                self.fill_buffer(dst_addr, LIBFABASYNC_MAGIC_1);
            }
        }
    }
    //#[tracing::instrument(skip_all)]
    ...
    fn check_buffer<T>(&self, dst_addr: &mut [T]) -> TxResult<()> {
        ...
        unsafe {
            if bytes_len % std::mem::size_of::<u64>() == 0 {
                self.check_buffer_elems(dst_addr, LIBFABASYNC_MAGIC_8)?;
            } else if bytes_len % std::mem::size_of::<u32>() == 0 {
                self.check_buffer_elems(dst_addr, LIBFABASYNC_MAGIC_4)?;
            } else if bytes_len % std::mem::size_of::<u16>() == 0 {
                self.check_buffer_elems(dst_addr, LIBFABASYNC_MAGIC_2)?;
            } else {
                self.check_buffer_elems(dst_addr, LIBFABASYNC_MAGIC_1)?;
            }
        }
        Ok(())
    }
    ...
    fn iget_data<T: Remote>(&self, pe: usize, src_addr: usize, dst_addr: &mut [T]) {
        ...
        match unsafe{
            async_std::task::block_on(async {self.ofi.lock().get(pe, src_addr, dst_addr, true).await})
        } {
...
impl CommOps for LibFabAsyncComm {
    ...
    fn alloc_pool(&self,min_size:usize) {
        ...
        let size = std::cmp::max(min_size * 2 * self.num_pes, LIBFABASYNC_MEM.load(Ordering::SeqCst));
    ...
    fn iput<T:Remote>(&self, pe:usize, src_addr: &[T],dst_addr:usize) {
        if pe != self.my_pe {
            async_std::task::block_on(async move {unsafe {self.ofi.lock().put(pe, src_addr, dst_addr, true)}.await.unwrap()});
            self.put_amt
                .fetch_add(src_addr.len() * std::mem::size_of::<T>(), Ordering::SeqCst);
        } else {
            unsafe {
    ...
    fn put_all<T:Remote>(&self, src_addr: &[T], dst_addr:usize) {
        async_std::task::block_on(async move { for pe in 0..self.my_pe {
            unsafe {self.ofi.lock().put(pe, src_addr, dst_addr, false)}.await.unwrap()
        }
...
impl LibFabAsyncData {
    //#[tracing::instrument(skip_all)]
    pub(crate) fn new(libfab_comm: Arc<Comm>, size: usize) -> Result<LibFabAsyncData, anyhow::Error> {
