...
use crate::{active_messaging::{AMCounters}, lamellae::{comm::
    rdma::{CommRdma, RdmaHandle, Remote}, CommAllocAddr, CommSlice}, LamellarTask};
...
impl CommRdma for ShmemComm {
    fn put<T: Remote>(&self, pe: usize, src: CommSlice<T>, remote_addr: CommAllocAddr) -> RdmaHandle<T> {
        ...
        for (addr, (shmem, size, addrs)) in alloc.0.iter() {
            if shmem.contains(remote_addr.0) {
                ...
                return ShmemFuture{op: Op::Put(pe,src,real_dst_addr)}.into();
            }
        }
    ...
    fn put_all<T: Remote>(&self, src: CommSlice<T>, remote_addr: CommAllocAddr) -> RdmaHandle<T> {
        ...
        for (addr, (shmem, size, addrs)) in alloc.0.iter() {
            if shmem.contains(remote_addr.0) {
                let pe_addrs = (0..self.num_pes).map(|pe| {
                    let real_dst_base = shmem.base_addr() + size * addrs[&pe].1;
                    let real_dst_addr = real_dst_base + (remote_addr.0 - addr);
                    (pe,real_dst_addr)
                }).collect::<Vec<_>>();
                return ShmemFuture{op: Op::PutAll(src,pe_addrs)}.into();
            }
        }
        panic!("shmem segment invalid for put all");
    }
    fn get<T: Remote>(&self, pe: usize, src_addr: CommAllocAddr, dst: CommSlice<T>) -> RdmaHandle<T> {
        ...
        for (addr, (shmem, size, addrs)) in alloc.0.iter() {
            if shmem.contains(src_addr.0) {
                ...
                return ShmemFuture{op: Op::Get(pe,real_src_addr,dst)}.into();
            }
        }
