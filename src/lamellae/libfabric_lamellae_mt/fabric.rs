...
use crate::{
    LAMELLAR_THREAD_ID, lamellae::{
        AllocationType, AtomicOp as LamellarAtomicOp, CollectiveOpKind, collective::{AllReduceOp, ReduceOp}, comm::{alloc::*, error::{AllocError, AllocResult, FabricError, FabricResult}}
    }, lamellar_alloc::{BTreeAlloc, LamellarAlloc}
};

use crate::{
    lamellae::{
        collective::{ReduceOp as LamellarReduceOp, RootOrSliceMut, RootSrcOrSliceMut, RootSrcSliceOrNone}}};
...
impl Ofi {
    ...
    fn collective_data_op_avail<T: 'static>(&self, data_op: CollectiveOp, reduce_op: Option<ReduceOp>) -> bool {
        let id = std::any::TypeId::of::<T>();
        if id == std::any::TypeId::of::<u8>() {
...
pub(crate) struct AllocInfoManager {
    pub(crate) mr_info_table: Arc<RwLock<Vec<LibfabricMtAlloc>>>,
...
impl AllocInfoManager {
    ...
    pub(crate) fn insert(&self, alloc: LibfabricMtAlloc) {
    ...
    pub(crate) fn remove_from_alloc(&self, mem_addr: &LibfabricMtAlloc) {
        let mut table = self.mr_info_table.write();
        if !table.is_empty() {
            let idx = table
                .iter()
    ...
    pub(crate) fn one_sided_alloc_from_remote_pe_and_addr(
        ...
        OneSidedLibfabricMtAlloc { alloc, remote_pe }.into()
    }
...
pub(crate) struct LibfabricMtAlloc {
...
impl std::fmt::Debug for LibfabricMtAlloc {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let mut temp = f.debug_struct("LibfabricMtAlloc");
...
impl Clone for LibfabricMtAlloc {
    fn clone(&self) -> Self {
        ...
        trace!(target: "libfabric", "Cloned LibfabricMtAlloc: {:?}", self);
        Self {
            ofi: self.ofi.clone(),
            mem: self.mem.clone(),
            mr: self.mr.clone(),
...
impl From<LibfabricMtAlloc> for CommAlloc {
    fn from(alloc: LibfabricMtAlloc) -> Self {
        CommAlloc {
            inner_alloc: CommAllocInner::LibfabricMtAlloc(alloc),
...
impl Drop for LibfabricMtAlloc {
    fn drop(&mut self) {
        ...
        // if self.print {
        //     println!(
        //         "[{:?}] Dropping LibfabricMtAlloc: {:x} - {:x} ref_cnt(before drop) {}",
        ...
        //     );
        // }
        debug!(target: "libfabric", "Dropping LibfabricMtAlloc: {:x} - {:x} ref_cnt(before drop) {}", self.range.start,self.range.end, fabric_ref_count);

        match &self.alloc_table {
            AllocTable::Fabric(alloc_table) => {
                if fabric_ref_count == 2 {
                    debug!(target: "libfabric", "Dropping fabric LibfabricMtAlloc: {:?}", self);
            ...
            AllocTable::Runtime(rt_alloc_table, addr, fabric_alloc_table) => {
                ...
                // if self.print {
                //     println!(
                //         "[{:?}, {:?}] Freeing runtime LibfabricMtAlloc: {:?}",
                ...
                // }
                if rt_ref_count == 1 {
                    debug!(target: "libfabric", "Freeing runtime LibfabricMtAlloc: {:?}",  self);
                ...
                if fabric_ref_count == 2 {
                    debug!(target: "libfabric", "Dropping fabric LibfabricMtAlloc from rt LibfabricMtAlloc: {:?}", self);
...
#[derive(Clone, Debug)]
pub(crate) struct OneSidedLibfabricMtAlloc {
    pub(crate) remote_pe: usize,
    pub(crate) alloc: LibfabricMtAlloc,
}

impl OneSidedLibfabricMtAlloc {
    pub(crate) fn num_bytes(&self) -> usize {
...
impl From<OneSidedLibfabricMtAlloc> for CommAlloc {
    fn from(alloc: OneSidedLibfabricMtAlloc) -> Self {
        CommAlloc {
            inner_alloc: CommAllocInner::OneSidedLibfabricMtAlloc(alloc),
