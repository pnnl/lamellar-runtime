...
use crate::lamellae::libfabric_async::libfabric_async_comm::{LibfabricAsyncComm, LibfabricAsyncData};
use crate::lamellae::{
...
pub(crate) struct LibfabricAsyncBuilder {
    my_pe: usize,
    num_pes: usize,
    libfab_comm: Arc<Comm>,
}

impl LibfabricAsyncBuilder {
    pub(crate) fn new(provider: &str, domain: &str) -> LibfabricAsyncBuilder {
        ...
        let libfab: Arc<Comm> = Arc::new(LibfabricAsyncComm::new(provider, domain).unwrap().into());
...
impl LamellaeInit for LibfabricAsyncBuilder {
    ...
    fn init_lamellae(&mut self, scheduler: Arc<Scheduler>) -> Arc<Lamellae> {
        ...
        let libfab = Arc::new(Lamellae::LibfabricAsync(libfab));
...
pub(crate) struct LibfabricAsync {
    my_pe: usize,
    num_pes: usize,
    libfab_comm: Arc<Comm>,
    active: Arc<AtomicU8>,
...
impl std::fmt::Debug for LibfabricAsync {
...
impl LibfabricAsync {
    fn new(my_pe: usize, num_pes: usize, libfab_comm: Arc<Comm>) -> LibfabricAsync {
...
impl LamellaeComm for LibfabricAsync {
...
#[async_trait]
impl LamellaeAM for LibfabricAsync {
...
impl Ser for LibfabricAsync {
    ...
    //     let ser_data = LibfabricAsyncData::new(self.rofi_comm.clone(), header_size + data_size)?;
    ...
    //     Ok(SerializedData::LibfabricAsyncData(ser_data))
    // }
    fn serialize_header(
        &self,
        header: Option<SerializeHeader>,
        ...
        let header_size = *SERIALIZE_HEADER_LEN;
        let ser_data =
            LibfabricAsyncData::new(self.libfab_comm.clone(), header_size + serialized_size)?;
...
impl LamellaeRDMA for LibfabricAsync {
    fn flush(&self) {
    ...
    fn alloc_pool(&self, min_size: usize) {
        ...
        match config().heap_mode {
            HeapMode::Static => {
                panic!("[LAMELLAR ERROR] Heap out of memory, current heap size is {} bytes,set LAMELLAR_HEAP_SIZE envrionment variable to increase size, or set LAMELLAR_HEAP_MODE=dynamic to enable exprimental growable heaps",LibfabricAsyncComm::heap_size())
            }
...
pub(crate) struct LibfabricAsyncFuture {}

impl Future for LibfabricAsyncFuture {
