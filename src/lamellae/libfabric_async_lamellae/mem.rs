...
use super::comm::{LibfabricAsyncComm, HEAP_SIZE};

use tracing::{debug, info, trace};

impl CommMem for LibfabricAsyncComm {
    ...
    fn alloc(
        ...
        let comm_alloc = CommAlloc {
            inner_alloc: Arc::new(CommAllocInner::LibfabricAsyncAlloc(inner_alloc)),
    ...
    //         }
    //         _ => {
    //             panic!("free should only be called with LibfabricAsyncAlloc or Raw addr");
    //         }
    //     }
    // }
    ...
    fn alloc_pool(&self, min_size: usize) {
        ...
        if let Ok(alloc) = self.alloc(size, AllocationType::Global, 0) {
            ...
            if let CommAllocInner::LibfabricAsyncAlloc(inner_alloc) = alloc.inner_alloc.as_ref() {
