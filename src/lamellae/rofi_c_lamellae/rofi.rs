...
pub(crate) fn rofi_c_remote_addr(pe: usize, local_addr: usize) -> AllocResult<usize> {
    ...
    if addr == 0 {
        ...
        Err(AllocError::LocalNotFound(CommAllocAddr(local_addr)))
    } else {
        Ok(addr)
    }
}
...
fn operand_ptr<T: Copy + 'static>(op: &mut AtomicOp<T>) -> *const T {
    match op {
        AtomicOp::Min(val)
        | AtomicOp::Max(val)
        | AtomicOp::Sum(val)
...
#[allow(dead_code)]
pub(crate) fn rofi_c_blocking_put<T>(src: &[T], dst: usize, pe: usize) -> RdmaResult {
    let src_addr = src.as_ptr() as *mut std::ffi::c_void;
    let size = src.len() * std::mem::size_of::<T>();
    let mut ret =
...
#[allow(dead_code)]
pub(crate) fn rofi_c_blocking_get<T>(src: usize, dst: &mut [T], pe: usize) -> RdmaResult {
    let src_addr = src as *mut std::ffi::c_void;
    let dst_addr = dst.as_ptr() as *mut std::ffi::c_void;
    let size = dst.len() * std::mem::size_of::<T>();
    // println!(
