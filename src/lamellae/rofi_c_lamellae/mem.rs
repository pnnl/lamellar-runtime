...
use super::{fabric::*,comm::{RofiCComm, ROFI_SIZE}};
...
impl CommMem for RofiCComm {
    ...
    fn local_addr(&self, remote_pe: usize, remote_addr: usize) -> CommAllocAddr {
        rofi_c_local_addr(remote_pe, remote_addr).expect("unable to locate local memory addr for remote addr").into() 
    }

    #[tracing::instrument(skip(self), level = "debug")]
    fn remote_addr(&self, pe: usize, local_addr: usize) -> CommAllocAddr {
        rofi_c_remote_addr(pe, local_addr).expect("unable to locate remote memory addr for local addr").into()
    }
