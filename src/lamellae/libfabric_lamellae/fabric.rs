use libfabric::{
    ...
    cq::{CompletionQueue, CompletionQueueBuilder, ReadCq},
    domain::{Domain, DomainBuilder},
    enums::{
        AVOptions, AddressFormat, AtomicOp, CollectiveOp, CollectiveOptions, CompareAtomicOp,
        EndpointType, FetchAtomicOp, HmemIface, JoinOptions, Mode, MrMode, Progress, ReduceOp,
        ResourceMgmt, TrafficClass, TransferOptions,
    },
...
use crate::{
    lamellae::{
        ...
        comm::{
            alloc::*,
            error::{AllocError, AllocResult, FabricError, FabricResult},
        },
        AllocationType, AtomicOp as LamellarAtomicOp, CollectiveOpKind,
    },
...
impl Ofi {
    pub(crate) fn new(provider: Option<&str>, domain: Option<&str>) -> FabricResult<Arc<Self>> {
        ...
        let unmapped_addresses: Vec<_> = my_pmi
            ...
            .iter()
            .map(|r| {
                let addr = my_pmi.get(&format!("epname"), &r).unwrap();
        ...
        for _ in 0..contexts_cache_size {
            contexts.lock().push(CachedContext {
    ...
    pub(crate) fn collective_avail<T: 'static>(&self, op: CollectiveOpKind) -> bool {
        match op {
            ...
            CollectiveOpKind::AllToAll => {
                self.collective_data_op_avail::<T>(CollectiveOp::AllToAll, None)
            }
            CollectiveOpKind::Broadcast => {
                self.collective_data_op_avail::<T>(CollectiveOp::Broadcast, None)
            }
            CollectiveOpKind::AllGather => {
                self.collective_data_op_avail::<T>(CollectiveOp::AllGather, None)
            }
            CollectiveOpKind::Gather => {
                self.collective_data_op_avail::<T>(CollectiveOp::Gather, None)
            }
            CollectiveOpKind::AllReduce(reduce_op) => {
                self.collective_data_op_avail::<T>(CollectiveOp::AllReduce, Some(reduce_op))
            }
            CollectiveOpKind::Reduce(reduce_op) => {
                self.collective_data_op_avail::<T>(CollectiveOp::Reduce, Some(reduce_op))
            }
            CollectiveOpKind::ReduceScatter(reduce_op) => {
                self.collective_data_op_avail::<T>(CollectiveOp::ReduceScatter, Some(reduce_op))
            }
            CollectiveOpKind::Scatter => {
                self.collective_data_op_avail::<T>(CollectiveOp::Scatter, None)
            }
        }
    }
    ...
    fn collective_data_op_avail<T: 'static>(
        &self,
        data_op: CollectiveOp,
        reduce_op: Option<LamellarReduceOp>,
    ) -> bool {
        let id = std::any::TypeId::of::<T>();
        if id == std::any::TypeId::of::<u8>() {
            ...
            self.domain.query_collective(data_op, &mut attr).is_ok()
        } else if id == std::any::TypeId::of::<u16>() {
...
impl LibfabricAlloc {
    ...
    pub(crate) fn atomic_compare_exchange_op_inner<T: 'static + Copy>(
        ...
        unsafe {
            if std::any::TypeId::of::<T>() == std::any::TypeId::of::<u8>() {
                ...
                self.typed_atomic_compare_exchange_op::<T, u128>(
                    pe, offset, current, new, result, blocking,
                )
            } else if std::any::TypeId::of::<T>() == std::any::TypeId::of::<usize>() {
    ...
    unsafe fn typed_allgather<T, OFI: AsFiType>(
        ...
        cg.post_collective(blocking, || {
            cg.ep
                .allgather(buf, None, res, None, mc, CollectiveOptions::default())
        })?;
    ...
    unsafe fn typed_alltoall<T, OFI: AsFiType>(
        ...
        cg.post_collective(blocking, || {
            cg.ep
                .alltoall(buf, None, res, None, mc, CollectiveOptions::default())
        })?;
    ...
    unsafe fn typed_scatter<T, OFI: AsFiType>(
        ...
        let (src, root_pe) = match src_or_root_pe {
            RootSrcSliceOrNone::Root(src) => (
                unsafe { std::mem::transmute::<&[T], &[OFI]>(src) },
                self.ofi.my_pe,
            ),
...
impl Drop for LibfabricAlloc {
    fn drop(&mut self) {
        trace!(target: "drop", "begin drop LibfabricAlloc");
        ...
        trace!(target: "drop", "end drop LibfabricAlloc");
    }
}
...
impl<T> From<&LamellarAtomicOp<T>> for FetchAtomicOp {
    fn from(op: &LamellarAtomicOp<T>) -> Self {
        match op {
            ...
            LamellarAtomicOp::Cas => panic!("Cas conversion to FetchAtomicOp not supported"),
