...
use crate::lamellae::rofi_rust::rofi_rust_comm::{RofiRustComm, RofiRustData};
use crate::lamellae::{
...
pub(crate) struct RofiRustBuilder {
    my_pe: usize,
    num_pes: usize,
    libfab_comm: Arc<Comm>,
}

impl RofiRustBuilder {
...
impl LamellaeInit for RofiRustBuilder {
    ...
    fn init_lamellae(&mut self, scheduler: Arc<Scheduler>) -> Arc<Lamellae> {
        ...
        let libfab = Arc::new(Lamellae::RofiRust(libfab));
...
impl std::fmt::Debug for RofiRust {
...
impl RofiRust {
    fn new(my_pe: usize, num_pes: usize, libfab_comm: Arc<Comm>) -> RofiRust {
...
impl LamellaeComm for RofiRust {
...
impl Ser for RofiRust {
    ...
    //     let ser_data = RofiRustData::new(self.rofi_comm.clone(), header_size + data_size)?;
    ...
    //     Ok(SerializedData::RofiRustData(ser_data))
    // }
    fn serialize_header(
        &self,
        header: Option<SerializeHeader>,
        ...
        let header_size = *SERIALIZE_HEADER_LEN;
        let ser_data = RofiRustData::new(self.libfab_comm.clone(), header_size + serialized_size)?;
...
impl LamellaeRDMA for RofiRust {
    ...
    fn alloc_pool(&self, min_size: usize) {
        ...
        match config().heap_mode {
            HeapMode::Static => {
                panic!("[LAMELLAR ERROR] Heap out of memory, current heap size is {} bytes,set LAMELLAR_HEAP_SIZE envrionment variable to increase size, or set LAMELLAR_HEAP_MODE=dynamic to enable exprimental growable heaps",RofiRustComm::heap_size())
            }
