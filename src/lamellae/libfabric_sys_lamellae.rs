...
impl LibfabricSysBuilder {
    pub(crate) fn new(provider: &str, domain: &str) -> LibfabricSysBuilder {
...
impl LamellaeInit for LibfabricSysBuilder {
    ...
    fn init_lamellae(&mut self, scheduler: Arc<Scheduler>) -> Arc<Lamellae> {
        ...
        let libfabric_sys = Arc::new(Lamellae::LibfabricSys(libfabric_sys));
...
impl std::fmt::Debug for LibfabricSys {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "LibfabricSys {{ my_pe: {}, num_pes: {},  active: {:?} }}",
...
impl LamellaeShutdown for LibfabricSys {
    fn shutdown(&self) {
...
#[async_trait]
impl LamellaeUtil for LibfabricSys {
