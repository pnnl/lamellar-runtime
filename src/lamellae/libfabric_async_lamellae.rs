...
impl LibfabricAsyncBuilder {
    pub(crate) fn new(provider: &str, domain: &str) -> LibfabricAsyncBuilder {
        ...
        let libfabric_comm: Arc<Comm> = Arc::new(LibfabricAsyncComm::new(provider, domain).into());
...
impl LamellaeInit for LibfabricAsyncBuilder {
    ...
    fn init_lamellae(&mut self, scheduler: Arc<Scheduler>) -> Arc<Lamellae> {
        let libfabric = LibfabricAsync::new(
            self.my_pe,
            self.num_pes,
        ...
        let libfabric = Arc::new(Lamellae::LibfabricAsync(libfabric));
...
impl std::fmt::Debug for LibfabricAsync {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "LibfabricAsync {{ my_pe: {}, num_pes: {},  active: {:?} }}",
...
impl LamellaeShutdown for LibfabricAsync {
    fn shutdown(&self) {
...
#[async_trait]
impl LamellaeUtil for LibfabricAsync {
