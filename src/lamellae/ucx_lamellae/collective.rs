use crate::{
    ...
    lamellae::collective::{
        BroadcastInput, CollectiveAllBroadcastIntoBufferOpFuture,
        CollectiveAllBroadcastIntoBufferOpHandle, CollectiveAllBroadcastOpFuture,
        CollectiveAllBroadcastOpHandle, CollectiveAllGatherIntoBufferOpFuture,
        ...
        CollectiveScatterOpHandle, CommAllocCollectiveAllBroadcast,
...
impl<T: Remote> UcxCollectiveAllReduceFuture<T> {
    fn exec_op(&mut self) {
        self.alloc
            .allreduce_inner(&self.op, self.src.as_slice(), &mut self.result, true)
            .unwrap();
        self.spawned = true;
    }
...
impl<T: Remote, B: AsLamellarBuffer<T>> UcxCollectiveAllReduceIntoBufferFuture<T, B> {
    fn exec_op(&mut self) {
        self.alloc
            .allreduce_inner(&self.op, self.src.as_slice(), self.result.as_mut_slice(), true)
            .unwrap();
        self.spawned = true;
    }
...
impl<T: Remote, B: AsLamellarBuffer<T>> UcxCollectiveAllReduceInPlaceFuture<T, B> {
    fn exec_op(&mut self) {
        self.alloc
            .allreduce_inplace_inner(&self.op, self.result.as_mut_slice(), true)
            .unwrap();
        self.spawned = true;
    }
...
impl<T: Remote> UcxCollectiveReduceFuture<T> {
    fn exec_op(&mut self) {
        self.alloc
            .reduce_inner(&self.op, self.src.as_slice(), self.target.as_mut_slice(), true)
            .unwrap();
        self.spawned = true;
    }
...
impl<T: Remote> UcxCollectiveAllGatherFuture<T> {
    fn exec_op(&mut self) {
        self.alloc
            .allgather_inner(self.src.as_slice(), &mut self.result, true)
            .unwrap();
        self.spawned = true;
    }
...
impl<T: Remote, B: AsLamellarBuffer<T>> UcxCollectiveAllGatherIntoBufferFuture<T, B> {
    fn exec_op(&mut self) {
        self.alloc
            .allgather_inner(self.src.as_slice(), self.result.as_mut_slice(), true)
            .unwrap();
        self.spawned = true;
    }
...
impl<T: Remote> UcxCollectiveGatherFuture<T> {
    fn exec_op(&mut self) {
        self.alloc
            .gather_inner(self.src.as_slice(), self.target.as_mut_slice(), true)
            .unwrap();
        self.spawned = true;
    }
...
#[pin_project(PinnedDrop)]
pub(crate) struct UcxCollectiveAllBroadcastFuture<T: Remote> {
    pub(crate) alloc: UcxAlloc,
    pub(crate) src: MemregionRdmaInputInner<T>,
    pub(crate) result: Vec<T>,
    pub(crate) scheduler: Arc<Scheduler>,
...
impl<T: Remote> UcxCollectiveAllBroadcastFuture<T> {
    fn exec_op(&mut self) {
        self.alloc
            .alltoall_inner(self.src.as_slice(), &mut self.result, true)
            .unwrap();
        self.spawned = true;
    }
...
#[pinned_drop]
impl<T: Remote> PinnedDrop for UcxCollectiveAllBroadcastFuture<T> {
    fn drop(self: Pin<&mut Self>) {
        if !self.spawned {
            RuntimeWarning::DroppedHandle("a UcxCollectiveAllBroadcastFuture").print();
        }
    }
}

impl<T: Remote> Future for UcxCollectiveAllBroadcastFuture<T> {
    type Output = Vec<T>;
    fn poll(mut self: Pin<&mut Self>, _cx: &mut Context<'_>) -> Poll<Self::Output> {
        if !self.spawned {
            self.exec_op();
...
#[pin_project(PinnedDrop)]
pub(crate) struct UcxCollectiveAllBroadcastIntoBufferFuture<T: Remote, B: AsLamellarBuffer<T>> {
    pub(crate) alloc: UcxAlloc,
    pub(crate) src: MemregionRdmaInputInner<T>,
    pub(crate) result: LamellarBuffer<T, B>,
    pub(crate) scheduler: Arc<Scheduler>,
...
impl<T: Remote, B: AsLamellarBuffer<T>> UcxCollectiveAllBroadcastIntoBufferFuture<T, B> {
    fn exec_op(&mut self) {
        self.alloc
            .alltoall_inner(self.src.as_slice(), self.result.as_mut_slice(), true)
            .unwrap();
        self.spawned = true;
    }
...
#[pinned_drop]
impl<T: Remote, B: AsLamellarBuffer<T>> PinnedDrop for UcxCollectiveAllBroadcastIntoBufferFuture<T, B> {
    fn drop(self: Pin<&mut Self>) {
        if !self.spawned {
            RuntimeWarning::DroppedHandle("a UcxCollectiveAllBroadcastIntoBufferFuture").print();
        }
    }
}

impl<T: Remote, B: AsLamellarBuffer<T>> Future for UcxCollectiveAllBroadcastIntoBufferFuture<T, B> {
    type Output = ();
    fn poll(mut self: Pin<&mut Self>, _cx: &mut Context<'_>) -> Poll<Self::Output> {
        if !self.spawned {
            self.exec_op();
...
impl<T: Remote> UcxCollectiveBroadcastFuture<T> {
    fn exec_op(&mut self) {
        self.alloc.broadcast_inner(self.target.as_mut_slice(), true).unwrap();
        self.spawned = true;
    }
...
impl<T: Remote> UcxCollectiveScatterFuture<T> {
    fn exec_op(&mut self) {
        self.alloc
            .scatter_inner(&mut self.result, self.src_or_root_pe.as_slice(), true)
            .unwrap();
        self.spawned = true;
    }
...
impl<T: Remote, B: AsLamellarBuffer<T>> UcxCollectiveScatterIntoBufferFuture<T, B> {
    fn exec_op(&mut self) {
        self.alloc
            .scatter_inner(self.result.as_mut_slice(), self.src_or_root_pe.as_slice(), true)
            .unwrap();
        self.spawned = true;
    }
...
impl<T: Remote> UcxCollectiveReduceScatterFuture<T> {
    fn exec_op(&mut self) {
        self.alloc
            .reduce_scatter_inner(&self.op, self.src.as_slice(), &mut self.result, true)
            .unwrap();
        self.spawned = true;
    }
...
impl<T: Remote, B: AsLamellarBuffer<T>> UcxCollectiveReduceScatterIntoBufferFuture<T, B> {
    fn exec_op(&mut self) {
        self.alloc
            .reduce_scatter_inner(&self.op, self.src.as_slice(), self.result.as_mut_slice(), true)
            .unwrap();
        self.spawned = true;
    }
...
impl CommAllocCollectiveAllBroadcast for UcxAlloc {
    fn broadcast_all<T: Remote>(
        ...
        CollectiveAllBroadcastOpHandle {
            future: CollectiveAllBroadcastOpFuture::Ucx(UcxCollectiveAllBroadcastFuture {
                alloc: self.clone(),
                src: memregion_in,
                result: vec![T::default(); len * self.num_pes],
                scheduler: scheduler.clone(),
    ...
    fn broadcast_all_into_buffer<T: Remote, B: AsLamellarBuffer<T>>(
        ...
        CollectiveAllBroadcastIntoBufferOpHandle {
            future: CollectiveAllBroadcastIntoBufferOpFuture::Ucx(
...
impl CommAllocCollectiveBroadcast for UcxAlloc {
    fn broadcast<T: Remote>(
        ...
        let target = match src_or_pe {
            BroadcastInput::Root(memregion_rdma_input_inner) => {
                RootSrcOrBuffer::Root(memregion_rdma_input_inner.into())
            }
            BroadcastInput::NotRoot(len, root_pe) => {
