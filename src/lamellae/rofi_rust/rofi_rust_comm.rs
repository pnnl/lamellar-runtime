...
static ROFIRUST_MAGIC_8: u64 = 0b1101001110111100011111001001100100111110011001100011110111001011;
static ROFIRUST_MAGIC_4: u32 = 0b10001111100100110010011111001100;
static ROFIRUST_MAGIC_2: u16 = 0b1100100110010011;
static ROFIRUST_MAGIC_1: u8 = 0b10011001;
...
impl RofiRustComm {
    ...
    fn init_buffer<T>(&self, dst_addr: &mut [T]) {
        ...
        unsafe {
            if bytes_len % std::mem::size_of::<u64>() == 0 {
                self.fill_buffer(dst_addr, ROFIRUST_MAGIC_8);
            } else if bytes_len % std::mem::size_of::<u32>() == 0 {
                self.fill_buffer(dst_addr, ROFIRUST_MAGIC_4);
            } else if bytes_len % std::mem::size_of::<u16>() == 0 {
                self.fill_buffer(dst_addr, ROFIRUST_MAGIC_2);
            } else {
                self.fill_buffer(dst_addr, ROFIRUST_MAGIC_1);
            }
        }
    }
    //#[tracing::instrument(skip_all)]
    ...
    fn check_buffer<T>(&self, dst_addr: &mut [T]) -> TxResult<()> {
        ...
        unsafe {
            if bytes_len % std::mem::size_of::<u64>() == 0 {
                self.check_buffer_elems(dst_addr, ROFIRUST_MAGIC_8)?;
            } else if bytes_len % std::mem::size_of::<u32>() == 0 {
                self.check_buffer_elems(dst_addr, ROFIRUST_MAGIC_4)?;
            } else if bytes_len % std::mem::size_of::<u16>() == 0 {
                self.check_buffer_elems(dst_addr, ROFIRUST_MAGIC_2)?;
            } else {
                self.check_buffer_elems(dst_addr, ROFIRUST_MAGIC_1)?;
            }
        }
        Ok(())
    }
...
impl Drop for RofiRustComm {
    fn drop(&mut self) {
        ...
        if self.alloc.read().len() > 1 {
            println!("[LAMELLAR INFO] {:?} additional rt memory pools were allocated, performance may be increased using a larger initial pool, set using the LAMELLAR_MEM_SIZE envrionment variable. Current initial size = {:?}",self.alloc.read().len()-1, ROFIRUST_MEM.load(Ordering::SeqCst));
        }
        // let _lock = self.comm_mutex.write();
        self.barrier();
        // std::thread::sleep(std::time::Duration::from_millis(1000));
...
impl SubData for RofiRustData {
    ...
    fn sub_data(&self, start: usize, end: usize) -> SerializedData {
        ...
        SerializedData::RofiRustData(sub)
    }
}
