...
static ROFIRUSTASYNC_MAGIC_8: u64 = 0b1101001110111100011111001001100100111110011001100011110111001011;
static ROFIRUSTASYNC_MAGIC_4: u32 = 0b10001111100100110010011111001100;
static ROFIRUSTASYNC_MAGIC_2: u16 = 0b1100100110010011;
static ROFIRUSTASYNC_MAGIC_1: u8 = 0b10011001;

pub(crate) static ROFIRUSTASYNC_MEM: AtomicUsize = AtomicUsize::new(4 * 1024 * 1024 * 1024);
...
impl std::fmt::Debug for RofiRustAsyncComm {
...
impl RofiRustAsyncComm {
    pub(crate) fn new(provider: Option<&str>, domain: Option<&str>) -> Result<Self, libfabric::error::Error> {
        ...
        let total_mem = cmd_q_mem + RT_MEM + ROFIRUSTASYNC_MEM.load(Ordering::SeqCst);
    ...
    fn init_buffer<T>(&self, dst_addr: &mut [T]) {
        ...
        unsafe {
            if bytes_len % std::mem::size_of::<u64>() == 0 {
                self.fill_buffer(dst_addr, ROFIRUSTASYNC_MAGIC_8);
            } else if bytes_len % std::mem::size_of::<u32>() == 0 {
                self.fill_buffer(dst_addr, ROFIRUSTASYNC_MAGIC_4);
            } else if bytes_len % std::mem::size_of::<u16>() == 0 {
                self.fill_buffer(dst_addr, ROFIRUSTASYNC_MAGIC_2);
            } else {
                self.fill_buffer(dst_addr, ROFIRUSTASYNC_MAGIC_1);
            }
        }
    }
    //#[tracing::instrument(skip_all)]
    ...
    fn check_buffer<T>(&self, dst_addr: &mut [T]) -> TxResult<()> {
        ...
        unsafe {
            if bytes_len % std::mem::size_of::<u64>() == 0 {
                self.check_buffer_elems(dst_addr, ROFIRUSTASYNC_MAGIC_8)?;
            } else if bytes_len % std::mem::size_of::<u32>() == 0 {
                self.check_buffer_elems(dst_addr, ROFIRUSTASYNC_MAGIC_4)?;
            } else if bytes_len % std::mem::size_of::<u16>() == 0 {
                self.check_buffer_elems(dst_addr, ROFIRUSTASYNC_MAGIC_2)?;
            } else {
                self.check_buffer_elems(dst_addr, ROFIRUSTASYNC_MAGIC_1)?;
            }
        }
        Ok(())
    }
    ...
    pub(crate) fn heap_size() -> usize {
        ROFIRUSTASYNC_MEM.load(Ordering::SeqCst)
    }
}



impl CommOps for RofiRustAsyncComm {
    fn my_pe(&self) -> usize {
        self.my_pe
    }
    ...
    fn alloc_pool(&self,min_size:usize) {
        ...
        let size = std::cmp::max(min_size * 2 * self.num_pes, ROFIRUSTASYNC_MEM.load(Ordering::SeqCst));
    ...
    fn alloc(&self, size:usize, alloc:AllocationType) -> AllocResult<usize>  {
        match alloc {
            ...
            AllocationType::Global => {
                ...
                Ok(
                    async_std::task::block_on(async move {self.ofi.lock().sub_alloc(&pes, size).await.unwrap()}))
            }
    ...
    fn put_all<T:Remote>(&self, src_addr: &[T], dst_addr:usize) {
        async_std::task::block_on(async move { for pe in 0..self.my_pe {
            unsafe {self.ofi.lock().put(pe, src_addr, dst_addr)}.await.unwrap()
        }
    ...
    fn get<T:Remote>(&self, pe:usize, src_addr:usize, dst_addr: &mut [T]) {
        if pe != self.my_pe {
            async_std::task::block_on(async {unsafe{self.ofi.lock().get(pe, src_addr, dst_addr)}.await.unwrap()});
            self.get_amt
...
impl Drop for RofiRustAsyncComm {
    fn drop(&mut self) {
        ...
        if self.alloc.read().len() > 1 {
            println!("[LAMELLAR INFO] {:?} additional rt memory pools were allocated, performance may be increased using a larger initial pool, set using the LAMELLAR_MEM_SIZE envrionment variable. Current initial size = {:?}",self.alloc.read().len()-1, ROFIRUSTASYNC_MEM.load(Ordering::SeqCst));
        }
        // let _lock = self.comm_mutex.write();
        self.barrier();
        // std::thread::sleep(std::time::Duration::from_millis(1000));
...
#[derive(Debug)]
pub(crate) struct RofiRustAsyncData {
...
impl RofiRustAsyncData {
    //#[tracing::instrument(skip_all)]
    pub(crate) fn new(libfab_comm: Arc<Comm>, size: usize) -> Result<RofiRustAsyncData, anyhow::Error> {
...
impl SerializedDataOps for RofiRustAsyncData {
    //#[tracing::instrument(skip_all)]
...
impl Des for RofiRustAsyncData {
    //#[tracing::instrument(skip_all)]
...
impl SubData for RofiRustAsyncData {
    //#[tracing::instrument(skip_all)]
    fn sub_data(&self, start: usize, end: usize) -> SerializedData {
        let mut sub = self.clone();
        sub.data_start += start;
        sub.data_len = end - start;
        SerializedData::RofiRustAsyncData(sub)
    }
}

impl Clone for RofiRustAsyncData {
    //#[tracing::instrument(skip_all)]
    fn clone(&self) -> Self {
        unsafe {
...
impl Drop for RofiRustAsyncData {
    //#[tracing::instrument(skip_all)]
    fn drop(&mut self) {
