...
impl WorkStealing {
    pub(crate) fn new(
        ...
        let inner = Arc::new(AmeScheduler::WorkStealingInner(WorkStealingInner::new(stall_mark.clone())));
        let sched = WorkStealing {
            inner: inner.clone(),
            ame: Arc::new(ActiveMessageEngine::new( my_pe, inner.clone(),teams,stall_mark.clone())),
        };
        sched
    }
