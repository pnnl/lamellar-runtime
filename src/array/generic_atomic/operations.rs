...
impl<T: ElementOps> LocalReadOnlyOps<T> for GenericAtomicLocalData<T> {
    fn local_load(&mut self, idx_vals: impl Iterator<Item =  (usize, T)>) -> Vec<T> {
        idx_vals.map(|(i,val)| {
            self.at(i).load()
        }).collect()
    }
...
impl <T: ElementCompareEqOps> LocalCompareExchangeOps<T> for GenericAtomicLocalData<T> {
    fn local_compare_exchange(&mut self, idx_vals: impl Iterator<Item = (usize, T)>, current: T) -> Vec<Result<T,T>> {
        idx_vals.map(|(i,val)| {
            self.at(i).compare_exchange(current,val)
        }).collect()
    }
}

impl <T: ElementComparePartialEqOps> LocalCompareExchangeOpsEpsilon<T> for GenericAtomicLocalData<T> {
    fn local_compare_exchange_epsilon(&mut self, idx_vals: impl Iterator<Item = (usize, T)>, current: T, epsilon: T) -> Vec<Result<T,T>> {
        idx_vals.map(|(i,val)| {
            self.at(i).compare_exchange_epsilon(current,val,epsilon)
        }).collect()
    }
}