...
impl<T: ElementArithmeticOps + Default> GenericAtomicArray<T> {
...
impl<T: ElementBitWiseOps + Default> GenericAtomicArray<T> {
...
impl<T: ElementArithmeticOps + Default> GenericAtomicArray<T> {
    ...
    ///
    /// Like [`sum_all`](GenericAtomicArray::sum_all) but places the result into a caller-supplied
...
impl<T: ElementComparePartialEqOps + Default> GenericAtomicArray<T> {
    ...
    ///
    /// Like [`min_all`](GenericAtomicArray::min_all) but places the result into a caller-supplied
    /// [`LamellarBuffer`] instead of allocating one internally.
    /// Returns an [`ArrayCollectiveAllReduceIntoBufferHandle`] that must be `spawn()`ed or `block()`ed.
    ///
    /// # Safety
...
impl<T: ElementBitWiseOps + Default> GenericAtomicArray<T> {
    ...
    ///
    /// Like [`bit_and_all`](GenericAtomicArray::bit_and_all) but places the result into a caller-supplied
    /// [`LamellarBuffer`] instead of allocating one internally.
    /// Returns an [`ArrayCollectiveAllReduceIntoBufferHandle`] that must be `spawn()`ed or `block()`ed.
    ///
    /// # Safety
    ...
    ///
    /// Like [`bit_xor_all`](GenericAtomicArray::bit_xor_all) but places the result into a caller-supplied
    /// [`LamellarBuffer`] instead of allocating one internally.
    /// Returns an [`ArrayCollectiveAllReduceIntoBufferHandle`] that must be `spawn()`ed or `block()`ed.
    ///
    /// # Safety
    ...
    ///
    /// Like [`bit_or_all`](GenericAtomicArray::bit_or_all) but places the result into a caller-supplied
    /// [`LamellarBuffer`] instead of allocating one internally.
    /// Returns an [`ArrayCollectiveAllReduceIntoBufferHandle`] that must be `spawn()`ed or `block()`ed.
    ///
    /// # Safety
...
impl<T: ElementArithmeticOps + Default> GenericAtomicArray<T> {
    ...
    ///
    /// Like [`sum_at_pe`](GenericAtomicArray::sum_at_pe) but the caller supplies the destination buffer via
...
impl<T: ElementComparePartialEqOps + Default> GenericAtomicArray<T> {
    ...
    ///
    /// Like [`min_at_pe`](GenericAtomicArray::min_at_pe) but the caller supplies the destination buffer via
    /// `dst: RootOrLamellarBuffer` instead of having the runtime allocate one. The root PE is encoded
    /// in the `dst` value. Returns an [`ArrayCollectiveReduceIntoBufferHandle`] that must be
    /// `spawn()`ed or `block()`ed.
    ///
...
impl<T: ElementBitWiseOps + Default> GenericAtomicArray<T> {
    ...
    ///
    /// Like [`bit_and_at_pe`](GenericAtomicArray::bit_and_at_pe) but the caller supplies the destination buffer via
    /// `dst: RootOrLamellarBuffer` instead of having the runtime allocate one. The root PE is encoded
    /// in the `dst` value. Returns an [`ArrayCollectiveReduceIntoBufferHandle`] that must be
    /// `spawn()`ed or `block()`ed.
    ///
    ...
    ///
    /// Like [`bit_xor_at_pe`](GenericAtomicArray::bit_xor_at_pe) but the caller supplies the destination buffer via
    /// `dst: RootOrLamellarBuffer` instead of having the runtime allocate one. The root PE is encoded
    /// in the `dst` value. Returns an [`ArrayCollectiveReduceIntoBufferHandle`] that must be
    /// `spawn()`ed or `block()`ed.
    ///
    ...
    ///
    /// Like [`bit_or_at_pe`](GenericAtomicArray::bit_or_at_pe) but the caller supplies the destination buffer via
    /// `dst: RootOrLamellarBuffer` instead of having the runtime allocate one. The root PE is encoded
    /// in the `dst` value. Returns an [`ArrayCollectiveReduceIntoBufferHandle`] that must be
    /// `spawn()`ed or `block()`ed.
    ///
...
// impl<T: Dist + Default> GenericAtomicArray<T> {
...
impl<T: Dist + Default> GenericAtomicArray<T> {
    ...
    ///
    /// Like [`gather_all`](GenericAtomicArray::gather_all) but places the concatenated result into the
...
impl<T: Dist + Default> GenericAtomicArray<T> {
    ...
    ///
    /// Like [`gather_at_pe`](GenericAtomicArray::gather_at_pe) but places the concatenated result into the
...
impl<T: Dist + Default> GenericAtomicArray<T> {
    ...
    ///
    /// Like [`alltoall`](GenericAtomicArray::alltoall) but places the received data into the caller-supplied
    /// [`LamellarBuffer`] instead of allocating one internally.
...
impl<T: Dist + Default> GenericAtomicArray<T> {
    ...
    ///
    /// Like [`broadcast_from_pe`](GenericAtomicArray::broadcast_from_pe) but places received data into the
...
impl<T: Dist + Default> GenericAtomicArray<T> {
    ...
    ///
    /// Like [`scatter_from_pe`](GenericAtomicArray::scatter_from_pe) but places the received `len`-element
...
impl<T: ElementArithmeticOps + Default> GenericAtomicArray<T> {
    ...
    ///
    /// Like [`sum_scatter`](GenericAtomicArray::sum_scatter) but places the received result segment into
...
impl<T: ElementComparePartialEqOps + Default> GenericAtomicArray<T> {
    ...
    ///
    /// Like [`min_scatter`](GenericAtomicArray::min_scatter) but places the received result segment into
    /// the caller-supplied [`LamellarBuffer`] instead of writing into the local array.
    /// Returns an [`ArrayCollectiveReduceScatterIntoBufferHandle`] that must be `spawn()`ed or `block()`ed.
    ///
    /// # Safety
...
impl<T: ElementBitWiseOps + Default> GenericAtomicArray<T> {
    ...
    ///
    /// Like [`bit_and_scatter`](GenericAtomicArray::bit_and_scatter) but places the received result segment into
    /// the caller-supplied [`LamellarBuffer`] instead of writing into the local array.
    /// Returns an [`ArrayCollectiveReduceScatterIntoBufferHandle`] that must be `spawn()`ed or `block()`ed.
    ///
    /// # Safety
    ...
    ///
    /// Like [`bit_xor_scatter`](GenericAtomicArray::bit_xor_scatter) but places the received result segment into
    /// the caller-supplied [`LamellarBuffer`] instead of writing into the local array.
    /// Returns an [`ArrayCollectiveReduceScatterIntoBufferHandle`] that must be `spawn()`ed or `block()`ed.
    ///
    /// # Safety
    ...
    ///
    /// Like [`bit_or_scatter`](GenericAtomicArray::bit_or_scatter) but places the received result segment into
    /// the caller-supplied [`LamellarBuffer`] instead of writing into the local array.
    /// Returns an [`ArrayCollectiveReduceScatterIntoBufferHandle`] that must be `spawn()`ed or `block()`ed.
    ///
    /// # Safety
