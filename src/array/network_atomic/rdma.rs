...
impl<T: Dist> NetworkAtomicArray<T> {
    ...
    ///
    /// Uses a network-level RDMA atomic write operation on the target PE. The element is written
    /// to whichever PE owns that index according to the array's [`Distribution`].
    ...
    ///
    /// let array: NetworkAtomicArray<usize> = NetworkAtomicArray::new(&world, num_pes, Distribution::Block).block();
    ///
    /// if my_pe == 0 {
    ...
    ///
    /// No completion handle is returned; use [`NetworkAtomicArray::wait_all`] or a barrier to
    ...
    ///
    /// Each element is written using a network-level RDMA atomic write, but the buffer as a
    /// whole is not collectively atomic. The runtime distributes writes across PEs according to
    ...
    /// let num_pes = world.num_pes();
    ///
    /// let array: NetworkAtomicArray<usize> = NetworkAtomicArray::new(&world, num_pes * 10, Distribution::Block).block();
    ...
    ///
    /// Uses a network-level RDMA atomic write on the target PE.
    /// Returns an [`ArrayRdmaPutHandle`] that must be `spawn()`ed or `block()`ed.
    ///
    /// # One-sided Operation
    /// The calling PE initiates the remote transfer.
    ...
    ///
    /// Each element is written using a network-level RDMA atomic write, but the buffer as a
    /// whole is not collectively atomic.
    /// Returns an [`ArrayRdmaPutHandle`] that must be `spawn()`ed or `block()`ed.
    ///
    /// # Safety
    ...
    ///
    /// Uses a network-level RDMA atomic read on the target PE. Returns an [`ArrayRdmaGetHandle`]
    ...
    ///
    /// Uses a network-level RDMA atomic read on the target PE.
    ///
    /// # One-sided Operation
    /// The calling PE initiates the remote transfer.
    ///
    ...
    ///
    /// Each element is read using a network-level RDMA atomic read. The snapshot as a whole is
    /// not collectively atomic. Returns an [`ArrayRdmaGetBufferHandle`] that resolves to `Vec<T>`
    /// when `spawn()`ed or `block()`ed.
    ///
    /// # Safety
    ...
    ///
    /// Each element is read using a network-level RDMA atomic read. Returns an
    ...
    /// blocking the calling thread until the transfer completes.
    ///
    /// Each element is read using a network-level RDMA atomic read.
    ///
    /// # Safety
    /// Multi-element buffer transfers are not collectively atomic. `pe` must be a valid PE index
    /// and `offset..offset+num_elems` must be within bounds of that PE's local memory region.
