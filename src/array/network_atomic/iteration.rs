...
impl<T: Dist> DistributedIterator for NetworkAtomicDistIter<T> {
...
impl<T: Dist> IndexedDistributedIterator for NetworkAtomicDistIter<T> {
...
impl<T: Dist> LocalIterator for NetworkAtomicLocalIter<T> {
    type Item = NetworkAtomicElement<T>;
    type Array = NetworkAtomicArray<T>;
    fn init(&self, start_i: usize, cnt: usize, _s: Sealed) -> Self {
        let max_i = self.data.num_elems_local();
