...
trait NativeAtomicOps<T,A> {
    ...
    unsafe fn local_compare_exchange_epsilon(&self, val: *const T,res: *mut T, current: *const T, eps: *const T)-> bool;
}
...
impl<T: Dist > NativeAtomicLocalData<T>{
    fn inner_op<A: NativeAtomicOps<T,A>>(&self, idx_vals: impl Iterator<Item = (usize, T)>,results: &mut Option<Vec<T>>, op: unsafe fn(&A,*const T,*mut T)) {
        let data = self.as_slice::<A>().expect("Slice to be equivalent of underlying native atomic type");
        if let Some(results) = results.as_mut(){
            let mut new_res = idx_vals.map(|(i,val)| {
                let mut res = val;
    ...
    fn inner_compare_exchange<A: NativeAtomicOps<T,A>>(&self, idx_vals: impl Iterator<Item = (usize, T)>,current: T,op: unsafe fn(&A,*const T,*mut T,*const T)->bool) -> Vec<Result<T,T>> {
        let data = self.as_slice::<A>().expect("Slice to be equivalent of underlying native atomic type");
        idx_vals.map(|(i,val)| {
            let mut res = val;
            match unsafe{op(&data[i],&val as *const T,&mut res as *mut T, &current as *const T)} {
                true => Ok(res),
                false => Err(res)
            }
        }).collect()
    }
    fn inner_compare_exchange_epsilon<A: NativeAtomicOps<T,A>>(&self, idx_vals: impl Iterator<Item = (usize, T)>,current: T,epsilon: T,op: unsafe fn(&A,*const T,*mut T,*const T,*const T)->bool) -> Vec<Result<T,T>> {
        let data = self.as_slice::<A>().expect("Slice to be equivalent of underlying native atomic type");
        idx_vals.map(|(i,val)| {
            let mut res = val;
            match unsafe{op(&data[i],&val as *const T,&mut res as *mut T, &current as *const T,&epsilon as *const T)} {
                true => Ok(res),
                false => Err(res)
            }
        }).collect()
...
impl<T: Dist + ElementShiftOps> LocalShiftOps<T> for NativeAtomicLocalData<T> {
    fn local_fetch_shl(&mut self, idx_vals: impl Iterator<Item = (usize, T)>,fetch: bool) -> Option<Vec<T>> {
        local_op!(self, idx_vals, fetch, fetch_shl)
    }

    fn local_fetch_shr(&mut self, idx_vals: impl Iterator<Item = (usize, T)>,fetch: bool) -> Option<Vec<T>> {
...
impl<T: Dist + ElementCompareEqOps> LocalCompareExchangeOps<T> for NativeAtomicLocalData<T> {
    fn local_compare_exchange(&mut self, idx_vals: impl Iterator<Item = (usize, T)>,current: T) ->Vec<Result<T,T>> {
        let t = std::any::TypeId::of::<T>();
        if t == std::any::TypeId::of::<i8>() {
            self.inner_compare_exchange::<AtomicI8>(idx_vals,current, AtomicI8::local_compare_exchange)
        } else if t == std::any::TypeId::of::<i16>() {
            self.inner_compare_exchange::<AtomicI16>(idx_vals, current, AtomicI16::local_compare_exchange)
        } else if t == std::any::TypeId::of::<i32>() {
            self.inner_compare_exchange::<AtomicI32>(idx_vals, current,AtomicI32::local_compare_exchange)
        } else if t == std::any::TypeId::of::<i64>() {
            self.inner_compare_exchange::<AtomicI64>(idx_vals, current,AtomicI64::local_compare_exchange)
        } else if t == std::any::TypeId::of::<isize>() {
            self.inner_compare_exchange::<AtomicIsize>(idx_vals,current, AtomicIsize::local_compare_exchange)
        } else if t == std::any::TypeId::of::<u8>() {
            self.inner_compare_exchange::<AtomicU8>(idx_vals,current, AtomicU8::local_compare_exchange)
        } else if t == std::any::TypeId::of::<u16>() {
            self.inner_compare_exchange::<AtomicU16>(idx_vals, current, AtomicU16::local_compare_exchange)
        } else if t == std::any::TypeId::of::<u32>() {
            self.inner_compare_exchange::<AtomicU32>(idx_vals, current, AtomicU32::local_compare_exchange)
        } else if t == std::any::TypeId::of::<u64>() {
            self.inner_compare_exchange::<AtomicU64>(idx_vals,current, AtomicU64::local_compare_exchange)
        } else if t == std::any::TypeId::of::<usize>() {
            self.inner_compare_exchange::<AtomicUsize>(idx_vals,current,  AtomicUsize::local_compare_exchange)
        }
        else {
            panic!("invalid native atomic type!");
        }
...
impl<T: Dist + ElementComparePartialEqOps> LocalCompareExchangeOpsEpsilon<T> for NativeAtomicLocalData<T> {
    fn local_compare_exchange_epsilon(&mut self, idx_vals: impl Iterator<Item = (usize, T)>,current: T, eps: T) -> Vec<Result<T,T>> {
        let t = std::any::TypeId::of::<T>();
        if t == std::any::TypeId::of::<i8>() {
            self.inner_compare_exchange_epsilon::<AtomicI16>(idx_vals, current, eps, AtomicI16::local_compare_exchange_epsilon)
        } else if t == std::any::TypeId::of::<i32>() {
            self.inner_compare_exchange_epsilon::<AtomicI32>(idx_vals, current, eps,AtomicI32::local_compare_exchange_epsilon)
        } else if t == std::any::TypeId::of::<i64>() {
            self.inner_compare_exchange_epsilon::<AtomicI64>(idx_vals, current, eps,AtomicI64::local_compare_exchange_epsilon)
        } else if t == std::any::TypeId::of::<isize>() {
            self.inner_compare_exchange_epsilon::<AtomicIsize>(idx_vals,current, eps, AtomicIsize::local_compare_exchange_epsilon)
        } else if t == std::any::TypeId::of::<u8>() {
            self.inner_compare_exchange_epsilon::<AtomicU8>(idx_vals,current, eps, AtomicU8::local_compare_exchange_epsilon)
        } else if t == std::any::TypeId::of::<u16>() {
            self.inner_compare_exchange_epsilon::<AtomicU16>(idx_vals, current, eps, AtomicU16::local_compare_exchange_epsilon)
        } else if t == std::any::TypeId::of::<u32>() {
            self.inner_compare_exchange_epsilon::<AtomicU32>(idx_vals, current, eps, AtomicU32::local_compare_exchange_epsilon)
        } else if t == std::any::TypeId::of::<u64>() {
            self.inner_compare_exchange_epsilon::<AtomicU64>(idx_vals,current, eps, AtomicU64::local_compare_exchange_epsilon)
        } else if t == std::any::TypeId::of::<usize>() {
            self.inner_compare_exchange_epsilon::<AtomicUsize>(idx_vals,current, eps,  AtomicUsize::local_compare_exchange_epsilon)
        }
        else {
            panic!("invalid native atomic type!");
        }
