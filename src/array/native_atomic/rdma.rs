...
impl<T: Dist> NativeAtomicArray<T> {
    ...
    ///
    /// Uses a native hardware atomic store on the target PE. The element is written to whichever
    /// PE owns that index according to the array's [`Distribution`].
    ...
    ///
    /// No completion handle is returned; use [`NativeAtomicArray::wait_all`] or a barrier to
    ...
    ///
    /// Each element is written using a native hardware atomic store on the target PE, but the
    /// buffer as a whole is not collectively atomic. The runtime distributes writes across PEs
    ...
    ///
    /// Uses a native hardware atomic store on the target PE.
    /// Returns an [`ArrayRdmaPutHandle`] that must be `spawn()`ed or `block()`ed.
    ///
    /// # One-sided Operation
    /// The calling PE initiates the remote transfer.
    ...
    ///
    /// Each element is written using a native hardware atomic store, but the buffer as a whole
    /// is not collectively atomic.
    /// Returns an [`ArrayRdmaPutHandle`] that must be `spawn()`ed or `block()`ed.
    ///
    /// # Safety
    ...
    ///
    /// Uses a native hardware atomic load on the target PE. Returns an [`ArrayRdmaGetHandle`]
    ...
    /// collectively atomic. Returns an [`ArrayRdmaGetBufferHandle`] that resolves to `Vec<T>`
