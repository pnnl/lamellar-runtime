...
fn impl_ops_scalar_type_match(crabtime::pattern!($bytes:expr, $bytes_type:ty,$result:expr): _) {
    ...
    let (aritmetic_ops, bitwise_ops, shift_ops, read_ops, write_ops, comp_ex_op, comp_ex_eps_op,return_type) = if result == "false" { //false means not returning anything, true means returning a OneSidedMemoryRegion<u8>
        ...
        (aritmetic_ops, bitwise_ops, shift_ops, read_ops, write_ops, comp_ex_op, comp_ex_eps_op,return_type)
    } else {
        let aritmetic_ops = crabtime::quote!{
...
fn impl_ops_option_scalar_type_match(crabtime::pattern!($bytes:expr, $bytes_type:ty, $result:expr): _) {
    ...
    for ty in ["u8", "u16", "u32", "u64", "usize", "u128", "i8", "i16", "i32", "i64", "isize", "i128","f32","f64","bool"].iter() {
...
impl PackedIdxVal {
    ...
    pub(crate) fn push<T>(&mut self,idx:usize, value: T) {
        ...
        let idx_bytes = unsafe {
            std::slice::from_raw_parts(
                (&idx as *const usize) as *const u8,
                idx_byte_width,
            )
        };
    ...
    pub(crate) fn slice_as_iter<T>(idx_byte_width: usize, data: &[u8]) -> impl Iterator<Item=(usize, T)> + '_ {
        data.chunks_exact(idx_byte_width+std::mem::size_of::<T>()).map(move |chunk| {
...
impl LamellarAM for PodMultiIdxSingleValAm {
    async fn exec(&self) {
        ...
        let indices: Box<dyn Iterator<Item=usize> + Send + '_> = match self.idx_size {
            1 => Box::new(unsafe{ self.indices.get_buffer(0, num_bytes).await}.into_iter().map(|b| b as usize)),
            2 => Box::new(unsafe{ self.indices.clone().to_base::<u16>().get_buffer(0, num_bytes/2).await }.into_iter().map(|b| b as usize)),
            4 => Box::new(unsafe{ self.indices.clone().to_base::<u32>().get_buffer(0, num_bytes/4).await }.into_iter().map(|b| b as usize)),
            _ => Box::new(unsafe{ self.indices.clone().to_base::<u64>().get_buffer(0, num_bytes/8).await }.into_iter().map(|b| b as usize)),
        };
        if self.opt {
...
impl LamellarAM for UsizeSingleValMultiIdxAm {
    async fn exec(&self) {
        let num_bytes = self.indices.len();
        let indices = unsafe { self.indices.clone().to_base::<u32>().get_buffer(0, num_bytes/4).await };
