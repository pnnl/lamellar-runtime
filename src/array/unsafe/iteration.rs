...
use crate::array::iterator::local_iterator::for_each::LocalIterForEachHandle;
...
impl<T: Dist> LocalIteratorLauncher for UnsafeArray<T> {
    ...
    where
        ...
        F: Fn(I::Item) + SyncSend + Clone + 'static,
    {
        self.local_for_each_static(iter, IterConsumer::ForEach(op))
    }
    ...
    where
        ...
        F: Fn(I::Item) + SyncSend + Clone + 'static,
    {
        let consumer = IterConsumer::ForEach(op);
        match sched {
            Schedule::Static => self.local_sched_static(iter,consumer ),
            Schedule::Dynamic => self.local_sched_dynamic(iter,consumer),
            Schedule::Chunk(size) => self.local_sched_chunk(iter, consumer),
            Schedule::Guided => self.local_sched_guided(iter, consumer),
            Schedule::WorkStealing => self.local_sched_work_stealing(iter, consumer),
        }
    }
    ...
    {
        self.local_sched_static(iter, IterConsumer::Reduce(F))
    }
