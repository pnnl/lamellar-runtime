...
impl<T: Dist> NativeAtomicElement<T> {
    ...
    ///     let local_data = array.local_data();
    ///     let elem = local_data.at(0);
    ///     let result = elem.compare_exchange_epsilon(0.0, 1.0, 1e-9);
    /// }
    ///```
...
impl<T: Dist + ArrayOps + Default> AsyncTeamFrom<(Vec<T>, Distribution)> for NativeAtomicArray<T> {
...
impl<T: Dist + AmDist + ElementBitWiseOps + 'static> NativeAtomicArray<T> {
    #[doc(hidden)]
