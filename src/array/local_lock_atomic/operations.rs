...
type OpFn = fn(*const u8, LocalLockByteArray, usize) -> LamellarArcAm;

lazy_static! {
    static ref OPS: HashMap<(ArrayOpCmd, TypeId), OpFn> = {
        let mut map = HashMap::new();
        for op in crate::inventory::iter::<LocalLockArrayOp> {
...
#[doc(hidden)]
pub struct LocalLockArrayOp {
    pub id: (ArrayOpCmd, TypeId),
    pub op: OpFn,
}

crate::inventory::collect!(LocalLockArrayOp);

type BufFn = fn(LocalLockByteArray) -> Arc<dyn BufferOp>;
...
impl<T: AmDist + Dist + 'static> LocalLockArray<T> {
    fn initiate_op<'a>(
        &self,
        index: usize,
        val: T,
        ...
        // println!("initiate_op for LocalLockArray<T> ");
        if let Some(func) = OPS.get(&(op, TypeId::of::<T>())) {
            let array: LocalLockByteArray = self.clone().into();
